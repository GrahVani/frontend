      ...
      <div className="py-4 sm:py-6">
          ...
          {CATEGORIES.map((c, i) => (
            ...
            >
              ...
              <div>
                ...
                <div className="text-xs text-gray-600">{c.querent}</div>
              </div>
            </button>
          ))}
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              <div className="w-7 h-7 rounded-md flex items-center justify-center text-white text-sm" style={{ background: cat.color }}>
                {cat.icon}
              </div>
            ...
            <div className="grid grid-cols-3 gap-2 mb-2">
              <div className="p-2 rounded-lg bg-white border border-gray-100 text-center">
                <div className="text-xs text-gray-600 uppercase">Querent</div>
                <div className="text-sm font-bold text-gray-800">{cat.querent}</div>
              </div>
              <div className="p-2 rounded-lg bg-white border border-gray-100 text-center">
                <div className="text-xs text-gray-600 uppercase">Target</div>
                <div className="text-sm font-bold text-gray-800">{cat.target}</div>
              </div>
              <div className="p-2 rounded-lg bg-white border border-gray-100 text-center">
                <div className="text-xs text-gray-600 uppercase">Opponent</div>
                <div className="text-sm font-bold text-gray-800">{cat.opponent}</div>
              </div>
            </div>
            <div className="flex items-start gap-2">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 mt-0.5 shrink-0" />
              <span className="text-sm text-gray-700">{cat.winCondition}</span>
            </div>
            <div className="flex items-start gap-2 mt-1">
              <XCircle className="w-3.5 h-3.5 text-red-500 mt-0.5 shrink-0" />
              <span className="text-sm text-gray-700">{cat.loseCondition}</span>
            </div>
          </motion.div>
        </AnimatePresence>
        ...
        <div className="mb-5 p-4 rounded-2xl bg-white border border-gray-100 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-bold text-violet-800 uppercase tracking-wider">The Seed Method</span>
          </div>
          <div className="flex gap-2 mb-3">
            {SEED_RANGES.map((s, i) => (
              <button
                ...
                className={`flex-1 px-3 py-2 rounded-xl text-sm font-bold transition-all border ${seedSystem === i ? "shadow-sm" : ""}`}
                style={{
              ...
              >
                <div>{s.system}</div>
                <div className="text-xs font-normal opacity-70">{s.label}</div>
              </button>
            ))}
          </div>
          <div className="flex items-center gap-3">
            <div className="flex-1">
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-bold text-gray-700">Your Seed Number</span>
        ...
        <div className="mb-5 p-4 rounded-2xl bg-white border border-gray-100 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-bold text-violet-800 uppercase tracking-wider">Sincerity Check</span>
          </div>
          <div className="flex items-center gap-3 mb-3">
            <button
              onClick={() => { setSincerityCheck(true); setShowResult(false); }}
              className={`flex-1 px-3 py-2 rounded-xl text-sm font-bold transition-all border ${sincerityCheck ? "shadow-sm" : ""}`}
              style={{
          ...
          <div className={`p-2 rounded-lg text-center text-sm font-bold ${sincerityCheck ? "bg-emerald-50 text-emerald-800 border border-emerald-200" : "bg-red-50 text-red-800 border border-red-200"}`}>
