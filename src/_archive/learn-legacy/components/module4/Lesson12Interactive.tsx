...
import {
  ...
  Lightbulb, Play, Star, Zap, Flame, Clock,
} from "lucide-react";
...
import DashaTimelineExplorer from "@/components/learn/module4/DashaTimelineExplorer";
...
interface Lesson12InteractiveProps {
  lesson: Lesson;
...
const SIDEBAR_SECTIONS: SidebarSection[] = [
  ...
  { id: "sec-definition", label: "What is Vimshottari Dasha?", type: "definition", group: "Learn" },
...
const FLASHCARDS = [
  { id: "f1", front: "What is the Vimshottari Dasha?", back: "A mathematically calculated 120-year planetary timeline that divides a person's life into chapters and sub-chapters, each governed by a specific Graha (planet).", category: "Core Definition" },
  { id: "f2", front: "What does 'Vimshottari' mean in Sanskrit?", back: "Vimshati (20) + Shatam (100) = 120. It means the '120-Year Condition,' mapping the ideal human lifespan in Kali Yuga.", category: "Etymology" },
  { id: "f3", front: "What is the fixed sequence of Dasha lords and their years?", back: "Ketu (7) → Venus (20) → Sun (6) → Moon (10) → Mars (7) → Rahu (18) → Jupiter (16) → Saturn (19) → Mercury (17). Total = 120 years.", category: "Sequence" },
  { id: "f4", front: "How is the starting Dasha determined at birth?", back: "By the Nakshatra the Moon is transiting at birth. If born in Ashwini (ruled by Ketu), the native starts in Ketu Dasha — but only the remaining balance.", category: "Anchor Point" },
  { id: "f5", front: "What are the 5 fractal tiers of Vimshottari Dasha?", back: "1. Mahadasha (MD) — Great Period · 2. Antardasha (AD) — Sub-Period · 3. Pratyantardasha (PD) — Sub-Sub-Period · 4. Sookshmadasha (SD) — weeks · 5. Pranadasha — days/hours", category: "Fractal Tiers" },
  { id: "f6", front: "What is the Antardasha length formula?", back: "(MD years × AD years) ÷ 120. Multiply the Mahadasha lord's years by the Antardasha lord's years, then divide by 120.", category: "Math" },
];

const KNOWLEDGE_CHECKS = [
  { id: "kc1", question: "What is the total span of the Vimshottari Dasha system?", options: ["100 years", "108 years", "120 years", "144 years"], correctIndex: 2, explanation: "The Vimshottari Dasha spans exactly 120 years — Vimshati (20) + Shatam (100) = 120, the ideal lifespan in Kali Yuga." },
  { id: "kc2", question: "Which planet gets the longest Mahadasha period?", options: ["Jupiter", "Saturn", "Venus", "Rahu"], correctIndex: 2, explanation: "Venus gets 20 years — the longest single period in the Vimshottari sequence." },
  { id: "kc3", question: "What determines the starting Dasha at birth?", options: ["The Ascendant sign", "The Moon's Nakshatra", "The Sun's position", "The date of birth"], correctIndex: 1, explanation: "The Moon's Nakshatra at birth determines the starting Dasha. The balance remaining in that Nakshatra sets the initial period length." },
];
...
export default function Lesson12Interactive({ lesson, lessonProgress }: Lesson12InteractiveProps) {
  const { user } = useAuth();
  ...
  const recapItems = useMemo(() => {
    ...
    if (content.intro) {
      items.push({ id: "recap-intro", title: "What is Vimshottari Dasha?", summary: "A 120-year planetary timeline dividing life into chapters governed by specific Grahas — the ultimate cosmic operating system." });
    }
    content.sections?.forEach((s) => {
      if (s.type === "definition") items.push({ id: `recap-${s.id}`, title: s.title, summary: "The Dasha is a mathematically calculated timeline where each planet becomes the 'CEO' of life during its period." });
      if (s.type === "etymology") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Vimshati (20) + Shatam (100) = 120 years. The ideal human lifespan mapped as a cosmic clock." });
      if (s.type === "mechanics") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Fixed sequence, Moon's Nakshatra anchor point, and fractal tiers from Mahadasha down to Pranadasha." });
      if (s.type === "software_logic") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Software checks current Dasha lords against the query target house to find the highest-probability event window." });
    });
              ...
              <motion.div {...fadeUp} className="bg-white border border-amber-200/80 rounded-2xl p-6 sm:p-8 shadow-sm">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center">
                    ...
                    <div className="text-[10px] text-amber-600">Years Total</div>
                  </div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center">
                    <Star className="w-5 h-5 text-amber-600 mx-auto mb-1" />
                    <div className="text-lg font-bold text-gray-900">9</div>
                    <div className="text-[10px] text-amber-600">Planetary Lords</div>
                  </div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center">
                    <Zap className="w-5 h-5 text-amber-600 mx-auto mb-1" />
                    <div className="text-lg font-bold text-gray-900">5</div>
                    <div className="text-[10px] text-amber-600">Fractal Tiers</div>
                  </div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center">
                    <Target className="w-5 h-5 text-amber-600 mx-auto mb-1" />
                    <div className="text-lg font-bold text-gray-900">Moon</div>
                    <div className="text-[10px] text-amber-600">Anchor Point</div>
                  </div>
                </div>
              </motion.div>
              <CalloutBlock variant="important" className="mt-5">
                This lesson teaches the master clock of Vedic Astrology — the 120-year Vimshottari Dasha timeline. You will learn the sequence, the anchor point math, and how to code the fractal tiers into software.
              </CalloutBlock>
            </section>
                ...
                <div className="p-6 sm:p-8 bg-white rounded-2xl border-2 border-amber-200/60 shadow-sm">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xl font-bold text-gray-900">Continue to Gochara (Real-Time Transits)</p>
                      <p className="text-sm text-gray-500 mt-1">Next: Learn how live planetary movements trigger events in the birth chart.</p>
                    </div>
