        ...
        <div className="space-y-8 animate-in fade-in duration-700 pb-10">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-bronze font-serif text-sm">Ashtakoota Guna Milan & Dosha Analysis.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
                ...
                <div className="bg-softwhite border border-header-border/30 rounded-3xl p-6 text-center shadow-sm">
                    ...
                    <h2 className="text-xl font-bold font-serif text-ink">{clientDetails.name}</h2>
                    <p className="text-xs text-bronze uppercase tracking-widest mb-4">Primary Chart</p>
                ...
                <div className="bg-softwhite border border-dashed border-header-border/40 rounded-3xl p-6 text-center hover:bg-header-border/5 transition-colors cursor-pointer group relative">
                    ...
                    <h2 className="text-xl font-bold font-serif text-ink">{partnerName}</h2>
                    <p className="text-xs text-bronze uppercase tracking-widest mb-4">Partner Chart</p>
                    <div className="flex justify-center gap-2 text-sm">
            ...
            <div className="bg-softwhite border border-header-border/20 rounded-2xl overflow-hidden shadow-sm">
                <div className="p-4 bg-header-border/10 flex justify-between items-center">
                    <h3 className="font-serif font-bold text-ink">Ashtakoota Scorecard</h3>
                    <div className="px-4 py-1.5 bg-white rounded-lg shadow-sm border border-header-border/20">
                        <span className="text-xs font-bold text-bronze uppercase mr-2">Total Score</span>
                    ...
                    <tbody className="divide-y divide-header-border/10">
                        {GUNAS.map((guna, i) => (
                            <tr key={i} className={cn("hover:bg-ink/5 transition-colors", guna.obtained === 0 && "bg-red-50")}>
                                <td className="px-6 py-3 font-bold text-ink font-serif">{guna.name}</td>
                                ...
                                <td className="px-6 py-3 text-right font-mono text-bronze">{guna.max}</td>
                                <td className="px-6 py-3 text-right font-black text-ink">{guna.obtained}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
