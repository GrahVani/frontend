...
const getSeverityColor = (severity: string) => {
    switch (severity?.toLowerCase()) {
        ...
        default: return { badge: 'bg-ink/5 text-ink/50 border-ink/10', dot: 'bg-ink/30', bar: '#94A3B8' };
    }
};
        ...
        >
            <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="flex items-center gap-2 leading-none mb-1">
                            <h2 className="text-[20px] font-semibold text-ink">{current_dasha.planet}</h2>
                            <span className="text-[12px] tracking-[0.15em] text-ink/40">Mahadasha</span>
                        </div>
                ...
                <div className="flex flex-col items-end gap-1.5 min-w-[140px] max-w-[200px] flex-1">
                    <div className="flex justify-between w-full text-[11px] tracking-wider text-ink/40">
                        <span>Progress</span>
            ...
            <div className="mt-2.5 flex items-center gap-2 text-[12px] text-ink/40">
                <Star className="w-3.5 h-3.5" />
                <span>Birth Dasha Lord: <span className="text-ink/70">{birth_dasha_lord}</span></span>
            </div>
        </motion.div>
    );
};
    ...
    return (
        <div className={cn("p-3 relative overflow-hidden flex flex-col rounded-2xl border border-gold-primary/10", styles.glassPanel)}>
            <div className="flex items-center justify-between mb-3 px-1">
                <div>
                    <h3 className="text-[15px] font-semibold tracking-tight flex items-center gap-2 text-ink/80">
                        <Zap className="w-4 h-4 text-purple-600 fill-purple-600/20" />
                        Planetary Vigor
                    </h3>
                    <p className="text-[11px] tracking-widest text-ink/30 mt-0.5">Strength · Status · Afflictions</p>
                </div>
                <div className="flex items-center gap-3 text-[11px] tracking-widest">
                    <div className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100/50">
                        <TrendingUp className="w-3 h-3" />
                        <span>Strong</span>
                    </div>
                    <div className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-rose-50 text-rose-700 border border-rose-100/50">
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h4 className="text-[14px] font-semibold text-ink tracking-tight leading-none">{name}</h4>
                                </div>
                                <div className="text-right">
                                    <span className="text-[15px] font-semibold text-ink" style={{ color: barColor }}>{sa.current_rupas.toFixed(1)}</span>
                                    <span className="text-[10px] text-ink/30 ml-0.5">R</span>
                                </div>
                            </div>

                            {/* Rupa Bar */}
                            <div className="relative h-1.5 rounded-full bg-ink/5 overflow-hidden">
                                <motion.div
                                    initial={{ width: 0 }}
                            ...
                            {/* Functional Status */}
                            <div className={cn(
                                "text-[11px] px-2 py-1 rounded-md border text-center truncate",
                                getFunctionalRoleStyle(data.functional_role)
                            )}>
                                {data.functional_role === 'Benefic' ? 'Functional Benefic (Favorable)' :
                                    data.functional_role === 'Malefic' ? 'Functional Malefic (Unfavorable)' :
                                        data.functional_role || 'Neutral'}
                            </div>

                            {/* Afflictions */}
                            <div className="flex flex-wrap gap-1 min-h-[18px] content-start">
                                {sa.is_combust && (
                                    <span className="text-[11px] px-1.5 py-0.5 bg-rose-500/[0.04] text-rose-600 border border-rose-500/10 rounded flex items-center gap-1">
                                        <ShieldAlert className="w-3 h-3 shrink-0" /> Combust (Moudhya)
                                    </span>
                                )}
                                {sa.is_mathematically_weak && (
                                    <span className="text-[11px] px-1.5 py-0.5 bg-rose-500/[0.04] text-rose-600 border border-rose-500/10 rounded flex items-center gap-1">
                                        <TrendingDown className="w-3 h-3 shrink-0" /> Weak ({sa.current_rupas} &lt; {sa.required_rupas})
                                    </span>
                                )}
                                {!sa.is_combust && !sa.is_mathematically_weak && (
                                    <span className="text-[11px] italic text-ink/25 px-1">Neutral</span>
                                )}
                            </div>
    ...
    return (
        <div className={cn("p-3 relative overflow-hidden rounded-2xl border border-gold-primary/10", styles.glassPanel)}>
            <div className="flex items-center gap-2 mb-3 px-1">
                ...
                <div>
                    <h3 className="text-[15px] font-semibold text-ink tracking-tight leading-none mb-1">Active Doshas</h3>
                    <p className="text-[11px] tracking-wider text-ink/40">Karmic afflictions detected · with remedies</p>
                </div>
            </div>
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <span className={cn("text-[11px] tracking-wider px-2 py-0.5 rounded border", sevStyles.badge)}>
                                    {dosha.severity.toUpperCase()}
                                </span>
                            </div>

                            {/* Description */}
                            <p className="text-[12px] text-ink/60 leading-relaxed mb-2 px-1">{dosha.description}</p>

                            {/* Remedies for this dosha */}
                            {remedies && remedies.remedies.length > 0 && (
                                <div className="space-y-1.5 mt-2">
                                    <p className="text-[10px] tracking-widest text-ink/30 uppercase px-1">Prescribed Remedies</p>
                                    {remedies.remedies.map((remedy, i) => {
                                        const getIcon = (text: string) => {
                                            const t = text.toLowerCase();
                                            if (t.includes('puja') || t.includes('worship') || t.includes('tarpan')) return <Bell className="w-3.5 h-3.5 text-purple-500" />;
                                            if (t.includes('mantra') || t.includes('chalisa') || t.includes('recite')) return <Flame className="w-3.5 h-3.5 text-orange-500" />;
                                            if (t.includes('donate') || t.includes('feed')) return <HeartHandshake className="w-3.5 h-3.5 text-pink-500" />;
                                            return <HandHeart className="w-3.5 h-3.5 text-emerald-500" />;
                                        };
                                        return (
                                            <div key={i} className="flex items-center gap-2 px-2.5 py-2 bg-white shadow-sm rounded-lg border border-gold-primary/5 hover:border-gold-primary/20 transition-all">
                                                <div className="w-6 h-6 rounded-md bg-ink/5 flex items-center justify-center shrink-0">
                                                    {getIcon(remedy)}
                                                </div>
                                                <span className="text-[12px] text-ink/80 leading-tight">{remedy}</span>
                                            </div>
                                        );
                                    })}
                                </div>
    ...
    return (
        <div className={cn("p-3 relative overflow-hidden rounded-2xl border border-gold-primary/10", styles.glassPanel)}>
            <div className="flex items-center gap-2 mb-3 px-1">
                ...
                <div>
                    <h3 className="text-[15px] font-semibold text-ink tracking-tight leading-none mb-1">Planetary Remedies</h3>
                    <p className="text-[11px] tracking-wider text-ink/40">Mantras · Gemstones · Charity (Daana)</p>
                </div>
            </div>
                ...
                {planets.map(([planet, entry], idx) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-2">
                                    <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center text-white font-serif text-sm shadow-md bg-gradient-to-br", theme.gradient)}>
                                        {theme.symbol}
                                    </div>
                                    <div>
                                        <h4 className="text-[14px] font-semibold text-ink tracking-tight leading-none mb-1">{planet}</h4>
                                        <span className={cn(
                                            "text-[11px] px-2 py-0.5 rounded border inline-block",
                                ...
                                <div className="flex items-center gap-1.5">
                                    <span className="text-[11px] text-ink/30">STR</span>
                                    <span className="text-[15px] font-semibold text-ink">{sa.current_rupas.toFixed(1)}</span>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-2 flex-wrap">
                                {mantras.map((m, mi) => (
                                    <div key={`m-${mi}`} className="flex-1 min-w-[200px] bg-orange-50/40 border border-orange-100/50 rounded-lg p-2.5 hover:bg-orange-50/80 transition-colors">
                                        <div className="flex items-center gap-1.5 mb-1.5">
                                            <Flame className="w-3.5 h-3.5 text-orange-500" />
                                            <span className="text-[11px] tracking-wider text-orange-600">Mantra</span>
                                        </div>
                                        <p className="text-[12px] leading-snug text-ink italic mb-1.5 line-clamp-2">"{m.text}"</p>
                                        <div className="flex flex-wrap gap-1">
                                            <span className="text-[11px] px-2 py-0.5 rounded-md bg-amber-50 text-amber-700">
                                                {(m.count || 0).toLocaleString()} chants
                                            </span>
                                        </div>
                                        <p className="text-[10px] text-ink/40 mt-1.5 leading-snug italic">{m.rationale}</p>
                                    </div>
                                ))}
                                {gemstones.map((g, gi) => (
                                    <div key={`g-${gi}`} className="flex-1 min-w-[200px] bg-violet-50/40 border border-violet-100/50 rounded-lg p-2.5 hover:bg-violet-50/80 transition-colors">
                                        <div className="flex items-center gap-1.5 mb-1.5">
                                            <Gem className="w-3.5 h-3.5 text-violet-500" />
                                            <span className="text-[11px] tracking-wider text-violet-600">Gemstone</span>
                                        </div>
                                        <p className="text-[13px] font-semibold text-ink mb-1">{g.details?.primary || g.item}</p>
                                        {g.details?.weight && (
                                            <span className="text-[11px] px-2 py-0.5 rounded-md bg-violet-50 text-violet-700 inline-block mb-1">
                                                {g.details.weight}
                                            </span>
                                        )}
                                        <p className="text-[10px] text-ink/40 mt-1 leading-snug italic">{g.rationale}</p>
                                    </div>
                                ))}
                                {charities.map((c, ci) => (
                                    <div key={`c-${ci}`} className="flex-1 min-w-[200px] bg-pink-50/40 border border-pink-100/50 rounded-lg p-2.5 hover:bg-pink-50/80 transition-colors">
                                        <div className="flex items-center gap-1.5 mb-1.5">
                                            <HeartHandshake className="w-3.5 h-3.5 text-pink-500" />
                                            <span className="text-[11px] tracking-wider text-pink-600">Charity (Daana)</span>
                                        </div>
                                        <p className="text-[12px] text-ink/80 leading-snug mb-1.5">
                                            <span className="text-[10px] tracking-tight opacity-40 mr-0.5">Items:</span>
                                            {(c.items || []).join(', ')}
                                        </p>
                                        <p className="text-[10px] text-ink/40 mt-1 leading-snug italic">{c.rationale}</p>
                                    </div>
                                ))}
                            </div>
                        </motion.div>
    ...
    return (
        <div className={cn("p-3 relative overflow-hidden rounded-2xl border border-gold-primary/10", styles.glassPanel)}>
            <div className="flex items-center gap-2 mb-3 px-1">
                ...
                <div>
                    <h3 className="text-[15px] font-semibold text-ink tracking-tight leading-none mb-1">Active Yogas</h3>
                    <p className="text-[11px] tracking-wider text-ink/40">Beneficial planetary combinations</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-2">
                {yogaList.map(([key, yoga], idx) => (
                    ...
                    >
                        ...
                        <p className="text-[12px] text-ink/60 leading-relaxed px-1 mb-2">{yoga.effect}</p>
                        <div className="flex items-center gap-1.5 px-1">
                            <span className="text-[10px] tracking-widest text-ink/30 uppercase">Planets:</span>
                            {yoga.planets.map(p => {
    ...
    return (
        <div className={cn("p-3 relative overflow-hidden rounded-2xl border border-gold-primary/10", styles.glassPanel)}>
            <div className="flex items-center gap-2 mb-3 px-1">
                ...
                <div>
                    <h3 className="text-[15px] font-semibold text-ink tracking-tight leading-none mb-1">Mahadasha Timeline</h3>
                    <p className="text-[11px] tracking-wider text-ink/40">Complete Vimshottari Dasha Sequence</p>
                </div>
            </div>
                        ...
                        <motion.div
                            ...
                            className={cn(
                                ...
                                isCurrent ? "bg-amber-50 border-amber-200 shadow-md ring-1 ring-amber-300/50" :
                                    isPast ? "bg-ink/[0.02] border-ink/5 opacity-60" :
                                        "bg-white/40 border-gold-primary/5 hover:bg-white/70"
                            )}
                        >
                            <div className={cn(
                                "w-6 h-6 rounded-md flex items-center justify-center text-white font-serif text-[11px] shadow-sm bg-gradient-to-br",
                                theme.gradient,
                                isPast && "opacity-50"
                            )}>
                                {theme.symbol}
                            </div>
                            <span className={cn("text-[11px] text-ink/80", isPast && "text-ink/40")}>{dasha.planet.slice(0, 3)}</span>
                            <span className="text-[10px] text-ink/30">{dasha.duration_years}y</span>
                            <span className="text-[9px] text-ink/20">
    ...
    return (
        <div className={cn("p-3 relative overflow-hidden rounded-2xl border border-rose-100", styles.glassPanel)}>
            <div className="flex items-center gap-2 mb-3 px-1">
                ...
                <div>
                    <h3 className="text-[15px] font-semibold text-ink tracking-tight leading-none mb-1">Strict Warnings</h3>
                    <p className="text-[11px] tracking-wider text-ink/40">Critical gemstone restrictions</p>
                </div>
            </div>
            ...
            <div className="space-y-1.5">
                {warnings.map((warning, idx) => (
                    <motion.div
                        ...
                        className="flex items-start gap-2 px-3 py-2.5 bg-rose-50/30 border border-rose-100 rounded-lg"
                    >
                        <XCircle className="w-4 h-4 text-rose-500 shrink-0 mt-0.5" />
                        <p className="text-[12px] text-rose-800 leading-snug">{warning}</p>
                    </motion.div>
                ))}
            </div>
        </div>
...
const MetadataFooter = ({ meta }: { meta: MetadataInfo }) => {
    return (
        <div className="flex items-center gap-4 flex-wrap px-3 py-2.5 bg-ink/[0.02] rounded-xl border border-gold-primary/5">
            <div className="flex items-center gap-1.5">
                <Info className="w-3.5 h-3.5 text-ink/30" />
                <span className="text-[11px] text-ink/40">Ascendant</span>
                <span className="text-[12px] font-semibold text-ink">{meta.ascendant}</span>
            </div>
            <div className="w-px h-3 bg-ink/10" />
            <div className="flex items-center gap-1.5">
                <span className="text-[11px] text-ink/40">Ayanamsa</span>
                <span className="text-[12px] font-semibold text-ink">{meta.ayanamsa}</span>
            </div>
            <div className="w-px h-3 bg-ink/10" />
            <div className="flex items-center gap-1.5">
                <span className="text-[11px] text-ink/40">Node</span>
                <span className="text-[12px] font-semibold text-ink">{meta.node_precision}</span>
            </div>
            <div className="w-px h-3 bg-ink/10" />
            <div className="flex items-center gap-1.5">
                <span className="text-[11px] text-ink/40">Strength</span>
                <span className="text-[12px] font-semibold text-ink truncate max-w-[220px]">{meta.strength_logic}</span>
            </div>
        </div>
    );
};
    ...
    return (
        <div className={cn("h-full overflow-hidden flex flex-col", styles.dashboardContainer)} style={{ margin: 0, borderRadius: '1rem' }}>
            ...
            <div className="flex items-center justify-between border-b border-gold-primary/20 px-5 py-2.5 shrink-0 bg-surface-warm/50">
                <div className="flex items-center gap-3">
                    <h2 className={cn(TYPOGRAPHY.sectionTitle, "text-[20px] font-semibold tracking-tight !mb-0")}>
                        Vedic <KnowledgeTooltip term="general_upaya">Remedies</KnowledgeTooltip>
                    </h2>
                    {metadata.ascendant && (
                        <span className="text-[11px] tracking-[0.15em] text-ink/30 bg-ink/[0.03] px-2 py-0.5 rounded">
