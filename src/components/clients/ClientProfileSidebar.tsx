...
export default function ClientProfileSidebar({ client, activeTab, onTabChange }: ClientProfileSidebarProps) {
    ...
    return (
        <aside className="w-full lg:w-72 h-full flex flex-col bg-header-gradient border-r border-header-border/20 pb-6 relative z-30 shadow-2xl">
            {/* Header / Back Link */}
            <div className="p-6 pb-2">
                <Link href="/clients" className="flex items-center gap-2 text-header-border hover:text-active-glow transition-colors mb-6 font-serif text-sm">
                ...
                <div className="flex flex-col items-center mb-8">
                    <div className="w-20 h-20 rounded-full border border-header-border/40 p-1 mb-3 relative">
                        <div className="w-full h-full rounded-full bg-ink flex items-center justify-center overflow-hidden border border-white/10">
                            {client.avatar ? (
                                ...
                                <span className="text-2xl font-serif text-active-glow">{(client.firstName || client.fullName || '?')[0]}</span>
                            )}
                        </div>
                    ...
                    <p className="text-header-border text-[10px] uppercase tracking-widest">#{client.id}</p>
                </div>
            </div>
                    ...
                    return (
                        <button
                            ...
                            className={cn(
                                ...
                                isActive
                                    ? "bg-gradient-to-r from-header-border/20 to-transparent text-active-glow border-l-2 border-header-border"
                                    : "text-white/50 hover:text-white hover:bg-white/5"
                            )}
                        >
                            <item.icon className={cn("w-4 h-4 transition-transform group-hover:scale-110", isActive ? "text-active-glow" : "text-header-border")} />
            ...
            {/* Footer / Meta */}
            <div className="px-6 py-4 border-t border-header-border/10 text-center">
