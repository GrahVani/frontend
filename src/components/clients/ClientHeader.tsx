...
export default function ClientHeader({ client }: ClientHeaderProps) {
    return (
        <div className="relative overflow-hidden bg-ink-abyss border-b border-gold-primary/20 shadow-2xl">
            {/* Subtle Gradient Glow */}
            <div className="absolute -left-20 -top-20 w-64 h-64 bg-gold-primary/10 rounded-full blur-3xl opacity-30 pointer-events-none" />
            ...
            <div className="max-w-[1600px] mx-auto px-6 py-6 lg:py-8 relative z-10">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <div className="flex items-center gap-6">
                        <div className="hidden sm:flex w-14 h-14 rounded-full bg-gradient-to-br from-gold-dark to-ink border border-active-glow/30 items-center justify-center text-white shadow-xl">
                            <span className={cn(TYPOGRAPHY.sectionTitle, "text-[24px] !font-bold !text-white !mb-0")}>{(client.firstName || client.fullName || '?').charAt(0)}</span>
                        </div>
                        <div className="flex-1">
                            <div className="flex items-center gap-3 mb-1.5">
                                <h1 className={cn(TYPOGRAPHY.sectionTitle, "text-[30px] !font-bold !text-white tracking-tight !mb-0")}>
