...
export function BhagyankaCalculator() {
  ...
  return (
    <div data-interactive="bhagyanka-calculator" style={{ display: "grid", gap: "1rem", color: INK_PRIMARY }}>
      {/* Header */}
      <section style={{ border: `1px solid ${HAIRLINE}`, borderRadius: 8, background: SURFACE, padding: "1rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", gap: "1rem", flexWrap: "wrap" }}>
          <div>
            <p style={eyebrowStyle}>Lesson 21.3.2 — Bhāgyāṅka Computation</p>
            <h2 style={{ margin: "0.2rem 0 0", color: TEAL, fontSize: "1.35rem" }}>
              Full-Birth-Date Destiny-Number Calculator
            </h2>
            <p style={{ margin: "0.45rem 0 0", color: INK_SECONDARY, lineHeight: 1.55, maxWidth: 780 }}>
              Compute Bhāgyāṅka from the complete birth-date. See both conventions, the graha-aṅka register, caveats, and the combined Mūlāṅka-to-Bhāgyāṅka lifetime-arc.
            </p>
          </div>
...
function CalculatorTab() {
  ...
  const result = isValid ? computeBhagyanka(dayNum, monthNum, yearNum) : null;
  ...
  const mulanka = isValid ? computeMulanka(dayNum) : null;
  const mulankaRegister = mulanka !== null ? getRegister(mulanka) : null;
  const arc = result && mulanka !== null && finalResult !== null ? getLifetimeArc(mulanka, finalResult) : null;
      ...
      <div style={{ border: `1px solid ${HAIRLINE}`, borderRadius: 8, background: SURFACE, padding: "1rem", display: "grid", gap: "0.75rem" }}>
        ...
        {!isValid && (
          <div style={{ color: VERMILION, fontSize: "0.85rem", fontWeight: 850 }}>
            Enter a valid date (day 1-31, month 1-12, year 1000-9999).
          </div>
        )}
      </div>
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "0.75rem" }}>
            <div style={{ border: `1px solid ${convention === "strict" ? PURPLE : HAIRLINE}`, borderRadius: 8, background: convention === "strict" ? `${PURPLE}0A` : SURFACE, padding: "1rem", display: "grid", gap: "0.3rem" }}>
              <div style={{ fontSize: "0.72rem", textTransform: "uppercase", letterSpacing: "0.08em", color: convention === "strict" ? PURPLE : INK_MUTED, fontWeight: 900 }}>Bhāgyāṅka</div>
            ...
            {mulanka !== null && mulankaRegister && (
              <div style={{ border: `1px solid ${BLUE}33`, borderRadius: 8, background: `${BLUE}0A`, padding: "1rem", display: "grid", gap: "0.3rem" }}>
                <div style={{ fontSize: "0.72rem", textTransform: "uppercase", letterSpacing: "0.08em", color: BLUE, fontWeight: 900 }}>Mūlāṅka (birth-day)</div>
                <div style={{ fontSize: "1.6rem", fontWeight: 950, color: BLUE, lineHeight: 1 }}>{mulanka}</div>
                <div style={{ fontSize: "0.85rem", color: INK_SECONDARY }}>
                  {"graha" in mulankaRegister ? mulankaRegister.graha : mulankaRegister.baseGraha}
                </div>
              </div>
            )}
          </div>
          ...
          {/* Lifetime arc */}
          {arc && (
            <div style={{ border: `1px solid ${TEAL}44`, borderRadius: 8, background: `${TEAL}0A`, padding: "1rem", display: "flex", gap: "0.6rem", alignItems: "flex-start" }}>
              <CheckCircle2 size={20} color={TEAL} style={{ flexShrink: 0, marginTop: 2 }} />
              <div>
                <div style={{ fontWeight: 950, color: TEAL, fontSize: "0.95rem", marginBottom: "0.2rem" }}>Mūlāṅka-to-Bhāgyāṅka lifetime-arc</div>
                <p style={{ margin: 0, color: INK_PRIMARY, lineHeight: 1.55, fontSize: "0.9rem" }}>{arc}</p>
              </div>
            </div>
          )}
          ...
          {/* Destiny-determinism refusal */}
          <div style={{ border: `1px solid ${VERMILION}44`, borderRadius: 8, background: `${VERMILION}0A`, padding: "0.85rem", display: "flex", gap: "0.5rem", alignItems: "flex-start" }}>
            <ShieldAlert size={18} color={VERMILION} style={{ flexShrink: 0, marginTop: 2 }} />
            <p style={{ margin: 0, color: INK_SECONDARY, lineHeight: 1.5, fontSize: "0.85rem" }}>
              <strong style={{ color: VERMILION }}>Destiny-determinism refusal:</strong> Bhāgyāṅka is chart-context trajectory orientation, not fixed fate. Refuse *your DESTINY is X; you cannot escape it* / *Bhāgyāṅka determines your life-outcomes* framings. Conscious-choice + life-circumstance + integration-practice shape how the trajectory unfolds.
            </p>
          </div>
          ...
          <div style={{ border: `1px solid ${GREEN}44`, borderRadius: 8, background: `${GREEN}0A`, padding: "0.85rem", display: "flex", gap: "0.5rem", alignItems: "flex-start" }}>
            ...
            <p style={{ margin: 0, color: INK_SECONDARY, lineHeight: 1.5, fontSize: "0.85rem" }}>
              <strong style={{ color: GREEN }}>Cross-system identity:</strong> Bhāgyāṅka {finalResult} is identical across Chaldean, Pythagorean, and Vedic systems because it is birth-date-derived, not name-derived. System-selection is irrelevant for Bhāgyāṅka.
            </p>
          </div>
        </>
      )}
    ...
    <section style={{ display: "grid", gap: "0.75rem" }}>
      <div style={{ border: `1px solid ${HAIRLINE}`, borderRadius: 8, background: SURFACE, padding: "1rem" }}>
        ...
        <p style={{ margin: "0.3rem 0 0", color: INK_SECONDARY, lineHeight: 1.5, fontSize: "0.88rem" }}>
          Four birth-dates covering master 33, cross-graha trajectory, Śani conflation refusal, and same-graha coherent trajectory.
        </p>
      </div>
        ...
        <article key={ex.id} style={{ border: `1px solid ${HAIRLINE}`, borderRadius: 8, background: SURFACE, padding: "1rem", display: "grid", gap: "0.55rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "0.75rem", flexWrap: "wrap" }}>
            <div style={{ fontWeight: 950, color: INK_PRIMARY, fontSize: "1rem" }}>
              {ex.label} — {String(ex.day).padStart(2, "0")}-{String(ex.month).padStart(2, "0")}-{ex.year}
            </div>
          </div>
  ...
  return (
    <section style={{ border: `1px solid ${HAIRLINE}`, borderRadius: 8, background: SURFACE, padding: "1rem", display: "grid", gap: "0.75rem" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
        ...
        <span style={{ fontWeight: 950, color: TEAL, fontSize: "1rem" }}>Discipline-compliant Bhāgyāṅka reading workflow</span>
      </div>
...
type TabKey = "calculator" | "examples" | "workflow";
