...
function RecommendationCard({ dashaKey, active }: { dashaKey: DashaClassKey; active: boolean }) {
  ...
  return (
    <div
      ...
      style={{
        background: active ? dasha.tint : SURFACE,
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="m-0 text-xs font-bold uppercase" style={{ color: dasha.color, letterSpacing: "0.08em" }}>
            {dasha.role}
          </p>
            ...
            <div key={row.key} className="flex items-center gap-3">
              <div
                ...
                style={{
                  ...
                  color: active ? row.color : INK_MUTED,
                }}
              >
                {index + 1}
              </div>
              <div className="min-w-0 flex-1 rounded-xl px-3 py-2" style={{ background: active ? row.tint : SURFACE_2 }}>
                <p className="m-0 text-sm font-bold" style={{ color: active ? row.color : INK_MUTED }}>
                  {row.name}
                </p>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
              ...
              {DASHA_CLASS_ROWS.map((row) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      gridTemplateColumns: "152px 152px 220px minmax(220px,1fr)",
                      background: active ? row.tint : SURFACE,
                      borderTop: `1px solid ${HAIRLINE}`,
                    }}
                  >
                    <div className="px-3 py-3">
                      <p className="m-0 text-sm font-bold" style={{ color: row.color }}>
                        {row.name}
                      </p>
        ...
        <div className="min-w-0 space-y-4">
          ...
          <section className="rounded-xl p-4" style={{ background: SURFACE, border: `1px solid ${HAIRLINE}` }}>
            ...
            <p className="mt-3 rounded-xl p-3 text-sm font-semibold leading-relaxed" style={{ background: SURFACE_2, color: INK_PRIMARY }}>
          ...
          <section className="rounded-xl p-4" style={{ background: SURFACE, border: `1px solid ${HAIRLINE}` }}>
            ...
            <div className="mt-3 space-y-3">
              <div className="rounded-xl p-3" style={{ background: dashaByKey("vimshottari").tint }}>
                <p className="m-0 text-sm font-bold" style={{ color: dashaByKey("vimshottari").color }}>
                  1. Vimshottari always first
                </p>
              </div>
              <div className="rounded-xl p-3" style={{ background: dashaByKey("ashtottari").tint }}>
                <p className="m-0 text-sm font-bold" style={{ color: dashaByKey("ashtottari").color }}>
                  2. Ashtottari only if Rahu condition holds
                </p>
              </div>
              <div className="rounded-xl p-3" style={{ background: dashaByKey("yogini").tint }}>
                <p className="m-0 text-sm font-bold" style={{ color: dashaByKey("yogini").color }}>
