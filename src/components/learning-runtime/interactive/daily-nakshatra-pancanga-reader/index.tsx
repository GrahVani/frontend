...
const ACTIVITIES: { key: string; label: string; symbol: string }[] = [
  { key: "travel", label: "Travel", symbol: "◆" },
  { key: "marriage", label: "Marriage", symbol: "♥" },
  { key: "medical", label: "Medical", symbol: "✚" },
  { key: "spiritual", label: "Spiritual", symbol: "✦" },
  { key: "business", label: "Business", symbol: "⚖" },
  { key: "hair", label: "Cutting hair", symbol: "✂" },
  { key: "construction", label: "Construction", symbol: "▲" },
];
...
export function DailyNakshatraPancangaReader() {
  const [sliderMin, setSliderMin] = useState(480);
  const [todayVara] = useState(1);
  ...
  return (
    <div
      ...
      data-interactive="daily-nakshatra-pancanga-reader"
    >
      <div className="mb-4">
        <h2 className="text-lg font-semibold" style={{ color: "var(--gl-ink-primary)" }}>
          <IAST>Daily Nakṣatra &amp; Pañcāṅga Reader</IAST>
        </h2>
      ...
      <div className="rounded-xl p-4 mb-5" style={{ background: "var(--gl-card-surface-solid, #FFF9F0)", border: "1px solid var(--gl-gold-hairline)" }}>
        ...
        <input type="range" min={0} max={1439} step={1} value={sliderMin} onChange={(e) => setSliderMin(Number(e.target.value))} className="w-full outline-none" style={{ accentColor: "var(--gl-gold-accent)" }} />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 mb-5">
        <div className="rounded-xl p-3 text-center" style={{ background: rc.bg, border: `2px solid ${rc.border}` }}>
          <p className="text-[10px] font-bold uppercase tracking-wider mb-1" style={{ color: rc.text }}>Nakṣatra</p>
          <p className="text-base font-semibold" style={{ fontFamily: "var(--font-cormorant), serif", color: "var(--gl-ink-primary)" }}>
            <IAST>{nakshatra.value}</IAST>
          </p>
          <Devanagari size="sm">{nakData.devanagari}</Devanagari>
          {nextNakshatra && (
            <p className="text-[10px] mt-1" style={{ color: "var(--gl-ink-muted)" }}>
              until {fmtTime(nextNakshatra.time)} → <IAST>{nextNakshatra.value}</IAST>
            </p>
          )}
        </div>
        <div className="rounded-xl p-3 text-center" style={{ background: "var(--gl-card-surface-solid)", border: "1px solid var(--gl-gold-hairline)" }}>
      ...
      <div className="rounded-xl p-4 mb-5" style={{ background: "var(--gl-card-surface-solid, #FFF9F0)", border: `2px solid ${rc.border}` }}>
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 rounded-full flex items-center justify-center text-base font-bold" style={{ background: rc.bg, color: rc.text, border: `2px solid ${rc.border}` }}>
                {nakData.num}
              </div>
              <div>
                ...
                <h4 className="text-xl font-semibold" style={{ fontFamily: "var(--font-cormorant), serif", color: "var(--gl-ink-primary)" }}>
                  <IAST>{nakshatra.value}</IAST> <span className="text-sm" style={{ color: "var(--gl-ink-muted)" }}>({nakData.ruler})</span>
                </h4>
              </div>
            </div>
            <Devanagari size="sm" style={{ marginBottom: 10 }}>{nakData.devanagari}</Devanagari>
          ...
          <div className="flex-1">
            <p className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: "var(--gl-gold-accent)" }}>Recommended Activities</p>
            <div className="space-y-1.5">
              {ACTIVITIES.map((act) => {
                const rating = activities[act.key];
                const color = rating === "good" ? "#2d7d46" : rating === "bad" ? "#A23A1E" : "#B8860B";
                const bg = rating === "good" ? "#E8F5EE" : rating === "bad" ? "#FDE8E5" : "#FDF6E3";
                ...
                return (
                  <div key={act.key} className="flex items-center gap-3 rounded-lg p-2" style={{ background: bg, border: `1px solid ${color}30` }}>
                    <span className="text-sm font-bold w-5 text-center" style={{ color }}>{act.symbol}</span>
                    <div className="flex-1">
      ...
      <div className="rounded-xl p-4" style={{ background: "var(--gl-card-surface-solid, #FFF9F0)", border: "1px solid var(--gl-gold-hairline)" }}>
        <h4 className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: "var(--gl-gold-accent)" }}>Classical Reasoning</h4>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
          <div className="rounded-lg p-3" style={{ background: "#E8F5EE", border: "1px solid #A8D4B8" }}>
            <p className="font-semibold mb-1" style={{ color: "#2d7d46" }}>Auspicious (Śubha)</p>
          ...
          <div className="rounded-lg p-3" style={{ background: "#FDF6E3", border: "1px solid #E8D5A3" }}>
            <p className="font-semibold mb-1" style={{ color: "#B8860B" }}>Mixed (Mishra)</p>
            <p style={{ color: "var(--gl-ink-secondary)" }}>
          ...
          <div className="rounded-lg p-3" style={{ background: "#FDE8E5", border: "1px solid #E8AFA8" }}>
