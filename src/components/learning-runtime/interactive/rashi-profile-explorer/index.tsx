...
function AttributeConstellationMandala({
  ...
  return (
    <svg viewBox="0 0 400 400" className="w-full" style={{ maxWidth: 400 }} role="img" aria-label={`Attribute Constellation Mandala for ${rashi.nameIAST}`}>
      ...
      <text x={cx} y={cy - 12} textAnchor="middle" dominantBaseline="middle" fill={rashi.color} fontSize={20} fontFamily="var(--font-devanagari)" fontWeight={700}>
        {rashi.nameDevanagari}
      </text>
      {/* Center: IAST name */}
      <text x={cx} y={cy + 5} textAnchor="middle" dominantBaseline="middle" fill="var(--gl-gold-accent)" fontSize={11} fontFamily="var(--font-cormorant)" fontWeight={600}>
        {rashi.nameIAST}
      </text>
      {/* Center: English name */}
      <text x={cx} y={cy + 18} textAnchor="middle" dominantBaseline="middle" fill="var(--gl-ink-muted)" fontSize={8}>
        ...
        return (
          ...
          >
            ...
            <text x={pos.x} y={pos.y + 10} textAnchor="middle" dominantBaseline="middle"
              fill={isActive ? rashi.color : isConnected ? rashi.color : "var(--gl-ink-secondary)"}
              fontSize={isActive ? 8 : 7} fontWeight={isActive ? 700 : 400}
            ...
            {isActive && (
              <text x={labelPos.x} y={labelPos.y} textAnchor="middle" dominantBaseline="middle"
                fill="var(--gl-ink-primary)" fontSize={8} fontWeight={600}
                style={{ pointerEvents: "none" }}
              >
                {getAttributeValue(rashi, node.key).length > 30
                  ? getAttributeValue(rashi, node.key).substring(0, 28) + "…"
  ...
  return (
    <div className="space-y-3">
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full" style={{ maxWidth: width }} role="img" aria-label={`Dignity Landscape for ${rashi.nameIAST} spanning from ${rashi.startDegree} to ${rashi.endDegree} degrees`}>
        ...
        {Array.from({ length: 7 }, (_, i) => {
          ...
          return (
            <g key={deg}>
              ...
              <text x={x} y={arcY + 30} textAnchor="middle" fill="var(--gl-ink-muted)" fontSize={7}>
        ...
        <text x={arcLeft} y={arcY - 24} textAnchor="middle" fill="var(--gl-ink-muted)" fontSize={8}>
          {rashi.startDegree}°
        </text>
        <text x={arcRight} y={arcY - 24} textAnchor="middle" fill="var(--gl-ink-muted)" fontSize={8}>
          {rashi.endDegree}°
        </text>
        ...
        {dignities.map((d, i) => {
          ...
          if (d.degree !== undefined) {
            ...
            return (
              <g key={i}>
                ...
                {/* Label */}
                <text x={x} y={arcY - 25} textAnchor="middle" fill={dc.text} fontSize={7} fontWeight={600}>
                  {d.graha} {d.type}
                </text>
                <text x={x} y={arcY - 16} textAnchor="middle" fill={dc.text} fontSize={7}>
                  {d.degree}°
                </text>
              </g>
            );
          ...
          if (d.degreeStart !== undefined && d.degreeEnd !== undefined) {
            ...
            return (
              <g key={i}>
                ...
                <text x={(x1 + x2) / 2} y={arcY + 1} textAnchor="middle" dominantBaseline="middle" fill={dc.text} fontSize={7} fontWeight={600}>
          ...
          return (
            <g key={i}>
              ...
              <text x={arcLeft + arcWidth / 2} y={arcY + 48} textAnchor="middle" fill={dc.text} fontSize={7}>
      ...
      <div className="flex gap-2 flex-wrap">
        {dignities.map((d, i) => {
          const dc = DIGNITY_COLORS[d.type];
          return (
            <span key={i} className="px-2.5 py-1 rounded-lg text-xs font-medium flex items-center gap-1" style={{ background: dc.bg, color: dc.text, border: `1px solid ${dc.border}` }}>
              {d.badge} {d.graha} {d.degree !== undefined ? `${d.degree}°` : d.degreeStart !== undefined ? `${d.degreeStart}°–${d.degreeEnd}°` : ""}
              <span style={{ opacity: 0.65 }}>{d.type}</span>
            </span>
          );
        })}
        {dignities.length === 0 && (
          <span className="text-xs px-2.5 py-1 rounded-lg" style={{ background: "var(--gl-surface-manuscript)", color: "var(--gl-ink-muted)" }}>
        ...
        {CLASSICAL_GRAHAS.map((g) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-xs font-medium" style={{ color: isActive ? color : "var(--gl-ink-primary)" }}>{g}</span>
              {dignity && (
                <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: DIGNITY_COLORS[dignity.type].bg, color: DIGNITY_COLORS[dignity.type].text, fontSize: 7 }}>
                  {dignity.type}
                </span>
              )}
            </motion.button>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="p-3 rounded-xl" style={{ background: "#C9A24D0C", border: "1px solid #C9A24D25" }}>
        <div className="text-xs font-semibold" style={{ color: "var(--gl-gold-accent)" }}>
      ...
      <svg viewBox="0 0 400 140" className="w-full" style={{ maxWidth: 400 }} role="img" aria-label={`180 degree axis mirror comparison between ${rashi.nameIAST} and ${oppositeRashi.nameIAST}`}>
        ...
        <text x={200} y={50} textAnchor="middle" fill="var(--gl-gold-accent)" fontSize={9} fontWeight={600}>180°</text>
        <text x={200} y={62} textAnchor="middle" fill="var(--gl-ink-muted)" fontSize={7}>{oppositePair.axis}</text>
        ...
        <text x={80} y={80} textAnchor="middle" fill="var(--gl-ink-primary)" fontSize={8} fontWeight={600}>
          {rashi.nameIAST}
        </text>
        <text x={80} y={92} textAnchor="middle" fill="var(--gl-ink-muted)" fontSize={7}>
        ...
        <text x={320} y={80} textAnchor="middle" fill="var(--gl-ink-primary)" fontSize={8} fontWeight={600}>
          {oppositeRashi.nameIAST}
        </text>
        <text x={320} y={92} textAnchor="middle" fill="var(--gl-ink-muted)" fontSize={7}>
        ...
        <table className="w-full text-sm" style={{ borderCollapse: "collapse" }} aria-label={`Comparison table between ${rashi.nameIAST} and ${oppositeRashi.nameIAST}`}>
          <thead>
            <tr>
              ...
              <th className="text-left p-2.5 text-xs" style={{ color: rashi.color, borderBottom: `2px solid ${rashi.color}40`, background: "var(--gl-surface-manuscript)" }}>{rashi.nameDevanagari} {rashi.nameIAST}</th>
              <th className="text-left p-2.5 text-xs" style={{ color: oppositeRashi.color, borderBottom: `2px solid ${oppositeRashi.color}40`, background: "var(--gl-surface-manuscript)" }}>{oppositeRashi.nameDevanagari} {oppositeRashi.nameIAST}</th>
            </tr>
          </thead>
          <tbody>
            {compareAttrs.map((row) => {
              ...
              return (
                <tr key={row.label}>
                  <td className="p-2.5 text-xs font-medium" style={{ color: "var(--gl-ink-muted)", borderBottom: "1px solid var(--gl-gold-hairline)" }}>{row.label}</td>
                  <td className="p-2.5 text-xs" style={{ color: "var(--gl-ink-primary)", borderBottom: "1px solid var(--gl-gold-hairline)", background: same ? "#C9A24D08" : "transparent" }}>
                    {row.a} {same && <span style={{ color: "#C9A24D" }}>✓</span>}
                  </td>
                  <td className="p-2.5 text-xs" style={{ color: "var(--gl-ink-primary)", borderBottom: "1px solid var(--gl-gold-hairline)", background: same ? "#C9A24D08" : "#A23A1E08" }}>
  ...
  return (
    <div className="w-full space-y-5" style={{ fontFamily: "var(--font-sans)" }}>
      ...
      <div className="p-5 rounded-2xl flex items-center gap-4 animate-fade-in" style={{ background: `linear-gradient(135deg, ${rashi.color}08 0%, ${rashi.color}18 100%)`, border: `2px solid ${rashi.color}35`, boxShadow: `0 6px 28px ${rashi.color}12` }}>
        ...
        <div>
          <h2 className="text-2xl font-semibold" style={{ fontFamily: "var(--font-cormorant)", color: "var(--gl-gold-accent)" }}>
        ...
        {SECTIONS.map((s) => (
          <motion.button
            ...
            style={{
              ...
              opacity: activeSection === s.key ? 1 : 0.75,
              boxShadow: activeSection === s.key ? `0 3px 16px ${rashi.color}35` : "none",
                      ...
                      >
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                          {ATTRIBUTE_NODES.map((n) => (
                            ...
                            >
                              <div className="text-sm mb-0.5">{n.icon}</div>
                        ...
                        <div className="p-3 rounded-lg" style={{ background: `${rashi.color}06`, border: `1px dashed ${rashi.color}25` }}>
                          <div className="text-xs uppercase tracking-wide mb-1" style={{ color: "var(--gl-ink-muted)" }}>Mnemonic</div>
                          <div className="text-sm italic" style={{ color: "var(--gl-ink-secondary)" }}>{rashi.mnemonic}</div>
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
      ...
      <div className="text-xs p-3 rounded-xl" style={{ background: "var(--gl-surface-twilight-glass)", border: "1px solid var(--gl-gold-hairline)", color: "var(--gl-ink-muted)" }}>
        Cross-references:{" "}
