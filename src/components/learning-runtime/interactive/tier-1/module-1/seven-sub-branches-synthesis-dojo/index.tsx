...
import { MATRIX_CELLS, DRILL_SCENARIOS, STREAMS, SUBBRANCHES, type MatrixCell, type DrillScenario } from "./data";
...
function emphasisBadge(emphasis: MatrixCell["emphasis"]) {
  switch (emphasis) {
    case "deep":
      return { label: "Deep", bg: "rgba(58, 140, 90, 0.15)", color: "#3A8C5A" };
    case "distinctive-deep":
      return { label: "Distinctive", bg: "rgba(79, 111, 168, 0.15)", color: "#4F6FA8" };
    case "moderate":
      return { label: "Moderate", bg: "rgba(232, 199, 114, 0.12)", color: "#9C7A2F" };
    case "light":
      return { label: "Light", bg: "rgba(120, 120, 120, 0.10)", color: "#888888" };
  }
}

export function SevenSubBranchesSynthesisDojo() {
  ...
  return (
    <button
      ...
      style={{
        ...
        color: active ? INDIGO : INK_SECONDARY,
...
function MatrixView({ reducedMotion }: { reducedMotion: boolean }) {
  ...
  return (
    <div>
              ...
              {SUBBRANCHES.map((sb) => (
                <th
                  ...
                  style={{
                    ...
                    color: sb.color,
                    textAlign: "center",
                    borderBottom: `2px solid ${sb.color}40`,
                  }}
                >
                  {sb.name}
                </th>
              ...
              <tr key={stream.slug}>
                ...
                {SUBBRANCHES.map((sb) => {
                  const cell = MATRIX_CELLS.find(
                    (c) => c.streamSlug === stream.slug && c.subBranchSlug === sb.slug
                  )!;
                  const badge = emphasisBadge(cell.emphasis);
                  const isExpanded = expanded === `${stream.slug}-${sb.slug}`;
                  return (
                    <td key={sb.slug} style={{ verticalAlign: "top" }}>
                      <button
                        type="button"
                        onClick={() => setExpanded(isExpanded ? null : `${stream.slug}-${sb.slug}`)}
                        ...
                        style={{
                          ...
                          borderRadius: "6px",
                          border: "none",
                          background: isExpanded ? "rgba(79,111,168,0.10)" : "rgba(255,255,255,0.03)",
                          cursor: "pointer",
                          textAlign: "left",
      ...
      >
        Click any cell to expand its detail. Three patterns: (1) All four streams have deep Jātaka;
        (2) KP has uniquely distinctive Praśna; (3) Parāśari has deep saṁhitā coverage via Bṛhat
        Saṁhitā.
      </p>
    </div>
  );
          ...
          return (
            <button
              ...
              style={{
                ...
                borderRadius: "8px",
                border: `1.5px solid ${
                  state === "correct" ? "#3A8C5A" : state === "wrong" ? "#C8412E" : "rgba(156,122,47,0.25)"
                }`,
                background:
                  state === "correct"
                    ? "rgba(58,140,90,0.08)"
              ...
              <div className="flex items-start gap-3">
                <span
                  style={{
                    ...
                    fontWeight: 700,
                    color:
                      state === "correct" ? "#3A8C5A" : state === "wrong" ? "#C8412E" : INK_MUTED,
                    background:
                      state === "correct"
                        ? "rgba(58,140,90,0.15)"
                        : state === "wrong"
        ...
        <div className="mt-5 flex items-center justify-between">
          <p
            style={{
              ...
              fontSize: "14px",
              fontWeight: 600,
              color: selectedOption?.isCorrect ? "#3A8C5A" : "#C8412E",
            }}
          >
