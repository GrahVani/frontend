...
const PART = 30 / 20; // 1°30′
// D20 start by modality (index % 3): movable → Meṣa(0), fixed → Dhanus(8), dual → Siṁha(4).
// NOTE: distinct from D16's [0,4,8] — same three fire signs, different pairing.
const START = [0, 8, 4];
const MODALITY = ["movable", "fixed", "dual"];

export function VimshamshaCalculator() {
  const [sign, setSign] = useState(0); // Meṣa (movable)
  const [deg, setDeg] = useState(5);

  const mod = sign % 3;
  const start = START[mod];
  const part = Math.min(Math.floor(deg / PART), 19); // 0..19 → 1st..20th
  const dest = (start + part) % 12;
  const fmt = (d: number) => `${Math.floor(d)}°${Math.round((d % 1) * 60).toString().padStart(2, "0")}′`;

  return (
    <div data-interactive="vimshamsha-calculator" style={{ display: "grid", gap: "1rem", color: INK_PRIMARY }}>
      <section style={{ border: `1px solid ${HAIRLINE}`, borderRadius: 8, background: SURFACE, padding: "1rem" }}>
        <p style={{ margin: 0, color: INK_MUTED, fontSize: "0.78rem", fontWeight: 900, letterSpacing: "0.06em", textTransform: "uppercase" }}>D20 Viṁśāṁśa calculator</p>
        <h2 style={{ margin: "0.2rem 0 0.2rem", color: GOLD, fontSize: "1.3rem" }}>Where does the devotion-part land?</h2>
        <p style={{ margin: 0, color: INK_MUTED, fontSize: "0.85rem", lineHeight: 1.5 }}>Each sign splits into twenty parts of 1°30′. The twenty count forward from a start sign set by modality: movable → Meṣa, fixed → Dhanus, dual → Siṁha (a different fire-sign pairing from the D16).</p>
      ...
      <section style={{ border: `1px solid ${GREEN}`, borderRadius: 8, background: `${GREEN}10`, padding: "1rem" }}>
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", marginBottom: "0.5rem" }}>
          {[
            { k: "Part", v: `${part + 1} of 20 (${fmt(part * PART)}–${fmt((part + 1) * PART)})` },
        ...
        <p style={{ margin: 0, fontWeight: 900, fontSize: "1.1rem", color: GREEN }}>→ D20 sign: {SIGNS[dest]}</p>
        ...
        <p style={{ margin: "0.5rem 0 0", color: INK_MUTED, fontSize: "0.78rem" }}>D20 magnifies <strong>spiritual practice, devotion (upāsanā) and the iṣṭa-devatā</strong> — read alongside the D1.</p>
      </section>
    </div>
  );
}
