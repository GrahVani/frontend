...
const DIGNITY_STYLES: Record<string, string> = {
    ...
    'Enemy': 'bg-ink/5 text-ink/55 border-ink/10',
};
...
const COLUMNS: DataGridColumn<DetailedPlanetInfo>[] = [
        ...
        render: (row) => (
            <div className="flex items-center gap-3">
                <div className={cn(
                    ...
                    row.isRetro ? "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]" : "bg-gold-primary"
                )} />
                <span className={cn(TYPOGRAPHY.planetName, "text-[18px] tracking-tight")}>{row.planet}</span>
    ...
    },
    {
        ...
        headerClassName: 'px-6',
        render: (row) => (
            <span className={cn(TYPOGRAPHY.value, "text-[12px] font-mono bg-ink/5 px-2 py-1 rounded border border-ink/10 font-normal")}>{row.degree}</span>
        ),
    },
    {
        key: 'sign',
        ...
        render: (row) => (
            <div className="flex flex-col gap-1 w-24">
                <div className="flex justify-between items-end">
                    <span className={cn(TYPOGRAPHY.value, "text-[12px]")}>{row.shadbala || 0}</span>
                    <span className={cn(TYPOGRAPHY.label, "text-gold-dark lowercase")}>Rupas</span>
                </div>
                <div className="w-full h-1.5 bg-ink/10 rounded-full overflow-hidden">
                    <div
                        className="h-full bg-gradient-to-r from-gold-dark to-bronze rounded-full"
    ...
    },
    {
        ...
        headerClassName: 'px-6',
        render: (row) => (
            <span className={cn(TYPOGRAPHY.value, "text-[12px] italic font-normal")}>{row.avastha || '\u2014'}</span>
        ),
    },
    {
        key: 'dignity',
    ...
    },
    {
        ...
        render: (row) => row.karaka ? (
            <span className={cn(TYPOGRAPHY.label, "bg-ink text-softwhite px-2 py-0.5 rounded border border-gold-primary/35 lowercase shadow-sm")}>
                {row.karaka}
            </span>
        ) : (
...
export default function PlanetaryAnalytics({ planets }: PlanetaryAnalyticsProps) {
    return (
        <div className="bg-surface-pure/60 border border-gold-primary/20 rounded-3xl overflow-hidden shadow-xl backdrop-blur-md group">
            <div className="p-6 border-b border-gold-primary/15 flex items-center justify-between bg-gradient-to-r from-gold-dark/10 to-transparent">
                <div className="flex items-center gap-3">
                    <Shield className="w-5 h-5 text-gold-dark" />
                    <h3 className={cn(TYPOGRAPHY.sectionTitle, "text-[14px] lowercase")}>Planetary State Mastery</h3>
                </div>
                <div className={cn(TYPOGRAPHY.label, "bg-gold-primary/10 text-gold-dark px-3 py-1 rounded-full border border-gold-primary/20 lowercase")}>
                    Live Computation
                </div>
            </div>
            <DataGrid
