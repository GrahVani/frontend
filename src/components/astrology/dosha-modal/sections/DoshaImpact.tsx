...
export const DoshaImpact = memo(function DoshaImpact({ data }: DoshaImpactProps) {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Impact List */}
            <div className="prem-card border border-red-100 p-5">
                ...
                <div className="space-y-3">
                    ...
                    {impacts.length === 0 && <p className="text-[10px] italic text-ink/55">General planetary analysis applies.</p>}
                </div>
            </div>

            {/* Analysis Summary */}
            <div className="prem-card border border-red-100 p-5 flex flex-col">
                <h3 className="font-serif font-bold text-ink mb-4 flex items-center gap-2 text-[11px] uppercase tracking-wider">
                    <Activity className="w-4 h-4 text-red-500" /> Professional Assessment
                </h3>
                <div className="flex-1 space-y-4">
                    {analyses.map((item, i) => (
                        <div key={i} className="p-4 bg-red-50/30 rounded-xl border border-red-100/50 italic text-[12px] leading-relaxed text-ink/80 relative overflow-hidden group">
                            <div className="absolute top-0 left-0 w-1 h-full bg-red-500/20 group-hover:bg-red-500 transition-colors" />
                            "{item.content}"
                        </div>
                    ))}
                    {analyses.length === 0 && (
                        <div className="p-4 bg-surface-warm/30 rounded-xl border border-gold-primary/15 border-dashed text-[10px] text-center italic text-ink/55">
