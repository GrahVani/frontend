...
export const YogaRemedies = memo(function YogaRemedies({ data }: YogaRemediesProps) {
    ...
    if (isCategorized(data)) {
        return (
            <div className="bg-surface-warm border border-gold-primary/15 rounded-2xl p-5 relative overflow-hidden">
                ...
                <div className="space-y-4">
                    {data.map((cat, ci) => (
                        <details key={ci} open={ci === 0}>
                            <summary className="cursor-pointer select-none flex items-center gap-2 text-[12px] font-bold text-gold-dark hover:text-gold-primary transition-colors group">
                                <ChevronDown className="w-3 h-3 transition-transform group-open:rotate-180" />
                                <span>{CATEGORY_ICONS[cat.category] ?? 'âœ¦'} {cat.category}</span>
                                <span className="text-[9px] text-ink opacity-60 ml-auto">{cat.items.length} items</span>
                            </summary>
                            <div className="flex flex-wrap gap-2 mt-2 pl-5">
                                {cat.items.map((item, i) => (
                                    <div
                                        key={i}
                                        className="flex items-start gap-2 px-3 py-1.5 bg-white border border-gold-primary/15 rounded-xl shadow-sm hover:border-gold-primary transition-colors cursor-default"
                                    >
                                        <CheckCircle2 className="w-3 h-3 text-green-500 shrink-0 mt-0.5" />
                                        <span className="text-[11px] font-medium text-ink leading-snug">{item}</span>
                                    </div>
                                ))}
                            </div>
                        </details>
        ...
        <div className="bg-surface-warm border border-gold-primary/15 rounded-2xl p-5 relative overflow-hidden">
            ...
            <div className="flex flex-wrap gap-2">
                {flatData.map((remedy, i) => (
                    <div
                        key={i}
                        className="flex items-center gap-2 px-3 py-1.5 bg-white border border-gold-primary/15 rounded-xl shadow-sm hover:border-gold-primary transition-colors cursor-default"
                    >
                        <CheckCircle2 className="w-3 h-3 text-green-500 shrink-0" />
                        <span className="text-[11px] font-medium text-ink">{remedy}</span>
                    </div>
                ))}
            </div>
        </div>
