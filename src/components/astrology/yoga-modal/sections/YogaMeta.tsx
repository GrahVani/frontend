...
export const YogaMeta = memo(function YogaMeta({ data }: YogaMetaProps) {
    ...
    return (
        <div className="flex items-center gap-4 flex-wrap text-[12px] bg-amber-50/40 px-4 py-2.5 rounded-xl border border-amber-200/60">
            {chips.map((chip, i) => (
                <React.Fragment key={i}>
                    {i > 0 && <div className="w-px h-3 bg-amber-500/15" />}
