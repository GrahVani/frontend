                ...
                <div className="flex items-center gap-3">
                    {onRefresh && (
                        <button
                            onClick={onRefresh}
                            disabled={isLoading}
                            className={cn("p-1.5 rounded-full hover:bg-black/5 text-primary transition-colors border border-gold-primary/15", isLoading && "animate-spin")}
                            title="Refresh RP"
                        >
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin p-4 space-y-5">
                {isLoading ? (
                    <div className="py-12 flex flex-col items-center justify-center space-y-3">
                        ...
                        <span className="text-[13px] font-sans text-primary font-medium animate-pulse">Calculating Ruling Planets...</span>
                    </div>
                ) : (
                    <>
                        {/* Unique Planets Hero */}
                        <div className="relative p-5 bg-gradient-to-br from-surface-warm to-white/80 rounded-xl border border-gold-primary/20 shadow-inner text-center overflow-hidden">
                            ...
                            <h4 className="text-[11px] text-gold-dark uppercase tracking-[0.2em] mb-3 font-semibold">Unique ruling planets by strength</h4>
                            <div className="flex flex-wrap justify-center gap-3">
                                {uniquePlanets.map((planet, idx) => (
                                    <div key={planet} className="group flex flex-col items-center">
                                        <div className="w-12 h-12 rounded-lg bg-white border border-gold-primary/20 shadow-sm flex items-center justify-center text-primary group-hover:border-gold-primary group-hover:scale-105 transition-all duration-300">
                                            <span className="text-[22px] font-serif">{PLANET_SYMBOLS[planet] || '☉'}</span>
                                        </div>
                                        <span className="mt-1.5 text-[13px] font-semibold text-primary leading-none">{planet}</span>
                                        <span className="text-[10px] text-primary uppercase tracking-tighter font-medium">Rank {idx + 1}</span>
                                    </div>
                                ))}
                                {uniquePlanets.length === 0 && <span className="text-primary/50 italic text-[13px] font-medium">No data available</span>}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                            ...
                            <div className="space-y-4">
                                <h4 className="flex items-center gap-2 text-[11px] uppercase tracking-widest border-b border-gold-primary/20 pb-2 text-primary font-semibold">
                                ...
                                <div className="space-y-2 bg-white/40 p-3 rounded-xl border border-gold-primary/15">
                                    <h5 className="text-[10px] text-gold-dark uppercase tracking-wider font-semibold">Lagna (ascendant)</h5>
                                ...
                                {/* Moon Group */}
                                <div className="space-y-2 bg-white/40 p-3 rounded-xl border border-gold-primary/15">
                                    <h5 className="text-[10px] text-gold-dark uppercase tracking-wider font-semibold">Moon (mind)</h5>
                                    <div className="flex flex-col space-y-1.5">
                                ...
                                {/* Day Group */}
                                <div className="space-y-2 bg-white/40 p-3 rounded-xl border border-gold-primary/15">
                                    <h5 className="text-[10px] text-gold-dark uppercase tracking-wider font-semibold">Time (day)</h5>
                                    <div className="flex flex-col space-y-1.5">
                                ...
                                <div className="relative pl-4 space-y-3">
                                    ...
                                    {Object.entries(strengthOrder).map(([rank, title]) => (
                                        <div key={rank} className="relative flex items-center gap-4 group">
                                            <div className={cn(
                                                "w-6 h-6 rounded-full border-2 border-gold-primary/20 bg-white z-10 flex items-center justify-center text-[10px] font-semibold",
                                                rank.includes('1') || rank.includes('strongest') ? "border-gold-primary text-gold-dark shadow-sm scale-110" : "text-primary"
                                            )}>
                                                {rank.charAt(0)}
                                            </div>
                                            <div className="flex flex-col">
                                                <span className={cn(
                                                    ...
                                                    rank.includes('1') || rank.includes('strongest') ? "text-primary font-semibold" : "text-primary opacity-80"
                                                )}>
                                                    {title}
                                                </span>
                                                <span className="text-[11px] text-primary uppercase tracking-tighter font-medium">
...
function ComponentRow({ label, value, highlight = false, termKey }: { label: string, value: string | undefined, highlight?: boolean, termKey?: string }) {
    return (
        <div className={cn(
            ...
            <span className="text-[11px] text-primary uppercase tracking-wider font-medium">{termKey ? <KnowledgeTooltip term={termKey}>{label}</KnowledgeTooltip> : label}</span>
            <div className="flex items-center gap-2">
                <span className="text-[14px] text-primary font-semibold">{value || '-'}</span>
                {highlight && <span className="text-[8px] bg-gold-primary text-white px-1.5 py-0.5 rounded-full uppercase font-semibold tracking-tighter shadow-sm">Strong</span>}
            </div>
        </div>
    );
}
...
function CalcBadge({ icon, label, value, mono = false, termKey }: { icon: React.ReactNode, label: string, value: string, mono?: boolean, termKey?: string }) {
    return (
        <div className="flex items-center gap-3 p-2 bg-white rounded-lg border border-gold-primary/15 shadow-sm">
            ...
            <div className="flex flex-col">
                <span className="text-[10px] text-primary uppercase tracking-widest font-medium">{termKey ? <KnowledgeTooltip term={termKey}>{label}</KnowledgeTooltip> : label}</span>
                <span className={cn("text-[13px] text-primary font-semibold", mono && "font-mono")}>{value}</span>
            </div>
        </div>
    );
}
