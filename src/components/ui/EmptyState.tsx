...
export default function EmptyState({ icon: Icon, title, description, action, className }: EmptyStateProps) {
    return (
        <div role="status" aria-label={title} className={cn("flex flex-col items-center justify-center py-16 px-4 text-center animate-in fade-in duration-500", className)}>
            {Icon && (
                <div className="w-16 h-16 rounded-full bg-parchment border border-antique flex items-center justify-center mb-4 shadow-sm">
                    <Icon className="w-7 h-7 text-gold-dark" aria-hidden="true" />
                </div>
            )}
            <h3 className="text-lg font-serif font-bold text-ink mb-1">{title}</h3>
            {description && <p className="text-sm text-muted-refined max-w-sm leading-relaxed">{description}</p>}
