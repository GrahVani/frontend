...
import {
  ...
  Lightbulb, Play, Star, Zap, Flame, PieChart,
} from "lucide-react";
...
import VargaSlicerExplorer from "@/components/learn/module5/VargaSlicerExplorer";
...
interface Lesson16InteractiveProps {
  lesson: Lesson;
...
const SIDEBAR_SECTIONS: SidebarSection[] = [
  ...
  { id: "sec-definition", label: "What is a Varga Chakra?", type: "definition", group: "Learn" },
  { id: "sec-etymology", label: "Etymology", type: "etymology", group: "Learn" },
  { id: "sec-mechanics", label: "The 3 Critical Vargas", type: "mechanics", group: "Learn" },
...
const FLASHCARDS = [
  { id: "f1", front: "What is a Varga Chakra?", back: "A harmonic, mathematical sub-chart generated by slicing the 30-degree Rashi into microscopic, equal fractions based on a planet's exact longitudinal degree.", category: "Core Definition" },
  { id: "f2", front: "What does 'Varga' and 'Chakra' mean?", back: "Varga = Division/Class/Harmonic. Chakra = Wheel/Chart. Together: 'Divisional Wheels' — specialized sub-wheels spun from the main chart.", category: "Etymology" },
  { id: "f3", front: "What is the Navamsha (D-9) and its math?", back: "30° ÷ 9 = 3°20' per slice. The most important chart after D-1. Represents soul strength, second half of life, and marriage mechanics.", category: "D-9" },
  { id: "f4", front: "What is the Dashamsha (D-10) and its use?", back: "30° ÷ 10 = 3°00' per slice. Used exclusively for professional life, karma, public status, and work environment analysis.", category: "D-10" },
  { id: "f5", front: "What is the Shashtiamsha (D-60) and its crucial rule?", back: "30° ÷ 60 = 0°30' per slice. The ultimate karmic DNA scan. INVALID if birth time is not accurate to the minute — changes every 2 minutes.", category: "D-60" },
  { id: "f6", front: "Why are Vargas called the 'Validation Checksum'?", back: "A planet exalted in D-1 might be debilitated in D-9. Vargas reveal the TRUE underlying strength that the main chart hides.", category: "Checksum" },
];

const KNOWLEDGE_CHECKS = [
  { id: "kc1", question: "How many primary Vargas exist in the Shodashavarga system?", options: ["9", "12", "16", "24"], correctIndex: 2, explanation: "Shodashavarga means 16-fold division. There are 16 primary Vargas, though software focuses on the 3 most critical: D-9, D-10, and D-60." },
  { id: "kc2", question: "Which Varga changes every 2 minutes of clock time?", options: ["Navamsha (D-9)", "Dashamsha (D-10)", "Shashtiamsha (D-60)", "All of them"], correctIndex: 2, explanation: "The D-60 relies on half-degree (0°30') fractions, so it changes every 2 minutes. Birth time must be accurate to the minute for D-60 to be valid." },
  { id: "kc3", question: "What is the slice size of the Navamsha (D-9)?", options: ["3°00'", "3°20'", "0°30'", "5°00'"], correctIndex: 1, explanation: "30° divided by 9 equals exactly 3°20' per Navamsha slice, matching the Nakshatra Pada size from Lesson 1.4." },
];
...
export default function Lesson16Interactive({ lesson, lessonProgress }: Lesson16InteractiveProps) {
  const { user } = useAuth();
  ...
  const recapItems = useMemo(() => {
    ...
    if (content.intro) {
      items.push({ id: "recap-intro", title: "What is a Varga Chakra?", summary: "A harmonic sub-chart created by mathematically dividing the 30° Rashi into microscopic fractions based on exact planetary degrees." });
    }
    content.sections?.forEach((s) => {
      if (s.type === "definition") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Varga Chakras are mathematical sub-charts that reveal the hidden details of any life domain." });
      if (s.type === "etymology") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Varga = Division, Chakra = Wheel. 'Divisional Wheels' — specialized sub-wheels from the main chart." });
      if (s.type === "mechanics") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Three critical Vargas: D-9 (Soul/Marriage), D-10 (Career), D-60 (Karmic DNA). Each slices the sign into progressively smaller fractions." });
      if (s.type === "software_logic") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Vargas act as the ultimate checksum. A planet exalted in D-1 may be debilitated in D-9 — revealing true underlying strength." });
    });
              ...
              <motion.div {...fadeUp} className="bg-white border border-amber-200/80 rounded-2xl p-6 sm:p-8 shadow-sm">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center">
                    ...
                    <div className="text-[10px] text-amber-600">Primary Vargas</div>
                  </div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center">
                    <Star className="w-5 h-5 text-amber-600 mx-auto mb-1" />
                    <div className="text-lg font-bold text-gray-900">D-9</div>
                    <div className="text-[10px] text-amber-600">Soul Matrix</div>
                  </div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center">
                    <Zap className="w-5 h-5 text-amber-600 mx-auto mb-1" />
                    <div className="text-lg font-bold text-gray-900">D-10</div>
                    <div className="text-[10px] text-amber-600">Career Engine</div>
                  </div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center">
                    <Target className="w-5 h-5 text-amber-600 mx-auto mb-1" />
                    <div className="text-lg font-bold text-gray-900">D-60</div>
                    <div className="text-[10px] text-amber-600">Karmic DNA</div>
                  </div>
                </div>
              </motion.div>
              <CalloutBlock variant="important" className="mt-5">
                This lesson teaches the microscope of Vedic Astrology. You will learn how divisional charts reveal what the main birth chart hides — the true strength of every planet.
              </CalloutBlock>
            </section>
                ...
                <div className="p-6 sm:p-8 bg-white rounded-2xl border-2 border-amber-200/60 shadow-sm">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xl font-bold text-gray-900">Continue to Ashtakavarga</p>
                      <p className="text-sm text-gray-500 mt-1">Next: Learn the point-based transit matrix that quantifies planetary strength.</p>
                    </div>
