...
import {
  ...
  Lightbulb, Play, Star, Zap, Volume2,
} from "lucide-react";
...
import MantraShastraExplorer from "./MantraShastraExplorer";
...
interface Lesson31InteractiveProps {
  lesson: Lesson;
...
const SIDEBAR_SECTIONS: SidebarSection[] = [
  ...
  { id: "sec-definition", label: "Mantra Shastra", type: "definition", group: "Learn" },
  { id: "sec-etymology", label: "Etymology", type: "etymology", group: "Learn" },
  { id: "sec-mechanics", label: "Bija Mantras", type: "mechanics", group: "Learn" },
...
const FLASHCARDS = [
  { id: "f1", front: "What is Mantra Shastra?", back: "The science of using specific sound vibrations (mantras) to create an acoustic field that harmonizes a planet's electromagnetic frequency in the native's biofield.", category: "Core Definition" },
  { id: "f2", front: "What does 'Mantra Shastra' mean?", back: "Mantra = Sacred Sound/Formula. Shastra = Science/Treatise. 'The Science of Sacred Sounds.'", category: "Etymology" },
  { id: "f3", front: "What is a Bija Mantra?", back: "A 'seed' mantra — the single-syllable root sound of a planet. Each of the 9 planets has a unique Bija mantra that encodes its specific vibrational signature.", category: "Bija Mantra" },
  { id: "f4", front: "What is the Sun's Bija Mantra?", back: "Om Hram Hrim Hraum Sah Suryaya Namah. The root Bija is 'Hram.' Chanting this activates the solar plexus and boosts confidence, authority, and vitality.", category: "Sun Bija" },
  { id: "f5", front: "What is the Saturn Bija Mantra?", back: "Om Pram Prim Praum Sah Shanaye Namah. The root Bija is 'Pram.' Used to mitigate delays, hardships, and structural breakdowns caused by Saturn.", category: "Saturn Bija" },
  { id: "f6", front: "What is the rule for mantra repetition count?", back: "The count should align with the planet's number: Sun = 6,000, Moon = 11,000, Mars = 10,000, Mercury = 9,000, Jupiter = 19,000, Venus = 16,000, Saturn = 23,000, Rahu = 18,000, Ketu = 17,000.", category: "Repetition" },
];

const KNOWLEDGE_CHECKS = [
  { id: "kc1", question: "What does a Bija Mantra represent?", options: ["A long prayer in Sanskrit", "The single-syllable root sound of a planet", "A gemstone prescription", "A house placement rule"], correctIndex: 1, explanation: "Bija = Seed. A Bija Mantra is the single-syllable root sound that encodes a planet's unique vibrational signature." },
  { id: "kc2", question: "Which planet requires the highest mantra repetition count?", options: ["Jupiter (19,000)", "Saturn (23,000)", "Venus (16,000)", "Sun (6,000)"], correctIndex: 1, explanation: "Saturn requires 23,000 repetitions — the highest count. Saturn is the slowest, heaviest planet, so its energetic realignment requires the most sustained vibrational input." },
  { id: "kc3", question: "How does mantra chanting physically affect the native?", options: ["It changes the birth chart", "It creates an acoustic field harmonizing the planet's frequency in the biofield", "It moves planets in the sky", "It erases past karma completely"], correctIndex: 1, explanation: "Mantra chanting creates an acoustic field that harmonizes the planet's electromagnetic frequency within the native's biofield. It does not change the chart — it changes how the native resonates with it." },
];
...
export default function Lesson31Interactive({ lesson, lessonProgress }: Lesson31InteractiveProps) {
  const { user } = useAuth();
  ...
  const recapItems = useMemo(() => {
    ...
    if (content.intro) items.push({ id: "recap-intro", title: "Mantra Shastra", summary: "The science of using specific sound vibrations to create an acoustic field that harmonizes a planet's frequency in the native's biofield." });
    content.sections?.forEach((s) => {
      if (s.type === "definition") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Mantras do not change the chart — they change how the native resonates with it. Sound is a physical intervention." });
      if (s.type === "etymology") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Mantra = Sacred Sound, Shastra = Science. 'The Science of Sacred Sounds.'" });
      if (s.type === "mechanics") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Each planet has a Bija (seed) mantra. Repetition counts align with planetary numbers. Saturn requires the most repetitions (23,000)." });
      if (s.type === "software_logic") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Software diagnoses the afflicted planet, selects the correct Bija mantra, and generates a personalized chanting protocol with count and timing." });
    });
            ...
            <section id="sec-overview" className="mb-6 scroll-mt-32">
              <motion.div {...fadeUp} className="bg-white border border-amber-200/80 rounded-2xl p-6 sm:p-8 shadow-sm">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center"><Volume2 className="w-5 h-5 text-amber-600 mx-auto mb-1" /><div className="text-lg font-bold text-gray-900">9</div><div className="text-[10px] text-amber-600">Bija Mantras</div></div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center"><Star className="w-5 h-5 text-amber-600 mx-auto mb-1" /><div className="text-lg font-bold text-gray-900">23k</div><div className="text-[10px] text-amber-600">Saturn Reps</div></div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center"><Zap className="w-5 h-5 text-amber-600 mx-auto mb-1" /><div className="text-lg font-bold text-gray-900">Sound</div><div className="text-[10px] text-amber-600">= Frequency</div></div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center"><Target className="w-5 h-5 text-amber-600 mx-auto mb-1" /><div className="text-lg font-bold text-gray-900">Biofield</div><div className="text-[10px] text-amber-600">Harmonizer</div></div>
                </div>
              </motion.div>
              <CalloutBlock variant="important" className="mt-5">This lesson teaches the acoustic approach to remediation. Mantras are not prayers — they are precise vibrational formulas that physically retune your biofield to match beneficial planetary frequencies.</CalloutBlock>
            </section>
              ...
              <motion.div {...fadeUp}>
                <div className="p-6 sm:p-8 bg-white rounded-2xl border-2 border-amber-200/60 shadow-sm">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div><p className="text-sm text-amber-600 mb-1 font-medium">🎉 Lesson Complete!</p><p className="text-xl font-bold text-gray-900">Continue to Dana & Seva</p><p className="text-sm text-gray-500 mt-1">Next: Learn the algorithmic karmic offset — using charity and service to neutralize planetary afflictions.</p></div>
