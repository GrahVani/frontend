...
import {
  ...
  Lightbulb, Play, Star, Zap, Flame, HelpCircle,
} from "lucide-react";
...
import PrashnaHoraryExplorer from "@/components/learn/module6/PrashnaHoraryExplorer";
...
interface Lesson22InteractiveProps {
  lesson: Lesson;
...
const SIDEBAR_SECTIONS: SidebarSection[] = [
  ...
  { id: "sec-definition", label: "Prashna Shastra", type: "definition", group: "Learn" },
  { id: "sec-etymology", label: "Etymology", type: "etymology", group: "Learn" },
  { id: "sec-mechanics", label: "Temporal Anchor & Seed", type: "mechanics", group: "Learn" },
  { id: "sec-software", label: "Query Routing", type: "software_logic", group: "Learn" },
...
const FLASHCARDS = [
  { id: "f1", front: "What is Prashna Shastra (Horary Astrology)?", back: "A branch of Jyotish where a chart is cast not for birth, but for the exact moment a question forms in the mind. The cosmic environment at that moment contains the DNA of the answer.", category: "Core Definition" },
  { id: "f2", front: "What does 'Prashna' and 'Shastra' mean?", back: "Prashna = Question/Query. Shastra = Science/Treatise. Together: 'The Science of Questions.'", category: "Etymology" },
  { id: "f3", front: "What is the Temporal Anchor in Prashna?", back: "The exact timestamp (to the millisecond) when the user submits the question, plus GPS coordinates. The chart is cast for the software's 'now' moment, not birth data.", category: "Temporal Anchor" },
  { id: "f4", front: "What is the Seed Method (Number Prashna)?", back: "The user selects a random number while focusing on the question. 1-108 maps to Nakshatra Padas for standard charts. 1-249 maps to KP Sub-Lords for KP-based Prashna.", category: "Seed Method" },
  { id: "f5", front: "What is the Golden Rule of Prashna?", back: "Prashna only works with genuine, burning desire. If the Moon is in Gandanta (first/last degree of a sign) or the Ascendant is afflicted, the chart reflects chaos — the question may be insincere.", category: "Sincerity" },
  { id: "f6", front: "In a theft Prashna, which houses represent what?", back: "House 1 = Owner (User) · House 4 = Stolen/Lost Item · House 7 = Thief. If 7th Lord > 1st Lord, thief gets away. If 4th Lord connects to 1st or 2nd, item is recovered.", category: "Theft Routing" },
];

const KNOWLEDGE_CHECKS = [
  { id: "kc1", question: "What data is used to cast a Prashna chart?", options: ["The user's birth date and time", "The exact timestamp and GPS coordinates when the question is asked", "The astrologer's birth data", "A random number only"], correctIndex: 1, explanation: "Prashna charts use the software's execution timestamp (to the millisecond) and GPS coordinates — not the user's birth data." },
  { id: "kc2", question: "In the Seed Method, what range maps to KP Sub-Lords?", options: ["1-12", "1-60", "1-108", "1-249"], correctIndex: 3, explanation: "For KP-based Prashna, the seed number ranges from 1-249, mapping directly to one of the 249 microscopic Sub-Lords." },
  { id: "kc3", question: "In a medical Prashna, which house represents the disease?", options: ["House 1", "House 4", "House 6", "House 10"], correctIndex: 2, explanation: "House 6 = Disease. House 1 = Patient. House 10 = Doctor. House 4 = Medicine. If 10th Lord > 6th Lord, treatment succeeds." },
];
...
export default function Lesson22Interactive({ lesson, lessonProgress }: Lesson22InteractiveProps) {
  const { user } = useAuth();
  ...
  const recapItems = useMemo(() => {
    ...
    if (content.intro) items.push({ id: "recap-intro", title: "Prashna Shastra", summary: "The science of casting charts for the birth of a question. The cosmic moment contains the mathematical DNA of the answer." });
    content.sections?.forEach((s) => {
      if (s.type === "definition") items.push({ id: `recap-${s.id}`, title: s.title, summary: "A chart cast for the exact moment a question forms — used for binary Yes/No answers on highly specific queries." });
      if (s.type === "etymology") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Prashna = Question. Shastra = Science. 'The Science of Questions.'" });
      if (s.type === "mechanics") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Temporal Anchor (server timestamp + GPS), Seed Method (random number 1-108 or 1-249), and Sincerity Check (Gandanta/Afflicted Ascendant)." });
      if (s.type === "software_logic") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Software routes queries to specialized actors: Theft (H1=Owner, H4=Item, H7=Thief), Medical (H1=Patient, H6=Disease, H10=Doctor), Lawsuits (H1=Plaintiff, H7=Defendant, H10=Judge)." });
    });
            ...
            <section id="sec-overview" className="mb-6 scroll-mt-32">
              <motion.div {...fadeUp} className="bg-white border border-amber-200/80 rounded-2xl p-6 sm:p-8 shadow-sm">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center"><HelpCircle className="w-5 h-5 text-amber-600 mx-auto mb-1" /><div className="text-lg font-bold text-gray-900">Now</div><div className="text-[10px] text-amber-600">Timestamp</div></div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center"><Star className="w-5 h-5 text-amber-600 mx-auto mb-1" /><div className="text-lg font-bold text-gray-900">Seed</div><div className="text-[10px] text-amber-600">Random Number</div></div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center"><Zap className="w-5 h-5 text-amber-600 mx-auto mb-1" /><div className="text-lg font-bold text-gray-900">Yes/No</div><div className="text-[10px] text-amber-600">Binary Answer</div></div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center"><Target className="w-5 h-5 text-amber-600 mx-auto mb-1" /><div className="text-lg font-bold text-gray-900">GPS</div><div className="text-[10px] text-amber-600">Location Anchor</div></div>
                </div>
              </motion.div>
              <CalloutBlock variant="important" className="mt-5">This is the capstone of Module 6. Prashna Shastra gives instant answers without needing a birth chart — the universe encodes the answer in the exact moment of inquiry.</CalloutBlock>
            </section>
              ...
              <motion.div {...fadeUp}>
                <div className="p-6 sm:p-8 bg-white rounded-2xl border-2 border-amber-200/60 shadow-sm">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div><p className="text-sm text-amber-600 mb-1 font-medium">🎉 Module 6 Complete!</p><p className="text-xl font-bold text-gray-900">You've Mastered the Specialized Operating Systems</p><p className="text-sm text-gray-500 mt-1">You've now covered Jaimini, KP, Tajik, and Prashna — the pro-level precision engines.</p></div>
