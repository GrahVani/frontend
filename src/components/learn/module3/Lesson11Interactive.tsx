...
import {
  ...
  Lightbulb, Play, Star, Zap, Flame, Triangle,
} from "lucide-react";
...
import TrinityExecutionDashboard from "./TrinityExecutionDashboard";
...
interface Lesson11InteractiveProps {
  lesson: Lesson;
  lessonProgress: LessonProgressData | null;
}

// ─── Static Data ──────────────────────────────────────────────
const SECTION_IDS = ["hero", "sec-overview", "sec-definition", "sec-etymology", "sec-pillars", "sec-scoring", "sec-knowledge", "sec-concepts", "sec-flashcards", "sec-recap", "sec-quiz", "sec-next"];

const SIDEBAR_SECTIONS: SidebarSection[] = [
  { id: "hero", label: "Introduction", type: "overview", group: "Start" },
  { id: "sec-overview", label: "Overview", type: "overview", group: "Start" },
  { id: "sec-definition", label: "The Trinity", type: "definition", group: "Learn" },
  { id: "sec-etymology", label: "Etymology", type: "etymology", group: "Learn" },
  { id: "sec-pillars", label: "Three Pillars", type: "mechanics", group: "Learn" },
  { id: "sec-scoring", label: "Scoring Matrix", type: "mechanics", group: "Learn" },
...
const FLASHCARDS = [
  { id: "f1", front: "What is the Trinity of Execution?", back: "Three-Point Check: Bhava + Bhavesha + Karaka", category: "Core Definition" },
  { id: "f2", front: "The Three Pillars", back: "1. Bhava (House/Stage) · 2. Bhavesha (House Lord/Manager) · 3. Karaka (Universal Significator/CEO)", category: "Pillars" },
  { id: "f3", front: "Score 3/3", back: "Unhindered Success", category: "Scoring" },
  { id: "f4", front: "Score 2/3", back: "Success with Friction", category: "Scoring" },
  { id: "f5", front: "Score 0/3", back: "Denial / Destruction", category: "Scoring" },
];

const KNOWLEDGE_CHECKS = [
  { id: "kc1", question: "What are the 3 pillars of the Trinity?", options: ["Graha, Rashi, Nakshatra", "Bhava, Bhavesha, Karaka", "Kendra, Trikona, Upachaya", "Sun, Moon, Ascendant"], correctIndex: 1, explanation: "The Trinity of Execution consists of three pillars: Bhava (the house/stage), Bhavesha (the house lord/manager), and Karaka (the universal significator/CEO)." },
  { id: "kc2", question: "What does Bhavesha mean?", options: ["The planet that aspects a house", "The planet that owns the sign in a house (the manager)", "The natural significator of a house", "The house itself"], correctIndex: 1, explanation: "Bhavesha is the planet that owns the sign falling in a particular house. It acts as the manager or landlord of that house's affairs." },
  { id: "kc3", question: "What score means 'Success with Friction'?", options: ["3/3", "2/3", "1/3", "0/3"], correctIndex: 1, explanation: "A score of 2/3 means two out of the three pillars (Bhava, Bhavesha, Karaka) are supporting the matter, leading to success but with obstacles or friction along the way." },
];
...
export default function Lesson11Interactive({ lesson, lessonProgress }: Lesson11InteractiveProps) {
  const { user } = useAuth();
  ...
  const recapItems = useMemo(() => {
    ...
    if (content.intro) {
      items.push({ id: "recap-intro", title: "The Trinity of Execution", summary: "A synthesis framework that checks three pillars — Bhava, Bhavesha, and Karaka — to judge whether a life matter will manifest successfully." });
    }
    content.sections?.forEach((s) => {
      if (s.type === "definition") items.push({ id: `recap-${s.id}`, title: s.title, summary: "The Trinity = Bhava (house/stage) + Bhavesha (lord/manager) + Karaka (significator/CEO). All three must be checked together." });
      if (s.type === "etymology") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Bhava = house, Bhavesha = house lord, Karaka = doer/significator. The words themselves reveal their roles." });
      if (s.type === "content" && s.title.includes("Pillar")) items.push({ id: `recap-${s.id}`, title: s.title, summary: "Three pillars work together: the stage (Bhava), its manager (Bhavesha), and the universal CEO (Karaka)." });
      if (s.type === "content" && s.title.includes("Scor")) items.push({ id: `recap-${s.id}`, title: s.title, summary: "3/3 = Unhindered Success · 2/3 = Success with Friction · 1/3 = Weak Manifestation · 0/3 = Denial or Destruction." });
    });
              ...
              <motion.div {...fadeUp} className="bg-white border border-amber-200/80 rounded-2xl p-6 sm:p-8 shadow-sm">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center">
                    <Triangle className="w-5 h-5 text-amber-600 mx-auto mb-1" />
                    <div className="text-lg font-bold text-gray-900">3</div>
                    <div className="text-[10px] text-amber-600">Pillars</div>
                  </div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center">
                    <Target className="w-5 h-5 text-amber-600 mx-auto mb-1" />
                    <div className="text-lg font-bold text-gray-900">4</div>
                    <div className="text-[10px] text-amber-600">Scores</div>
                  </div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center">
                    <Layers className="w-5 h-5 text-amber-600 mx-auto mb-1" />
                    <div className="text-lg font-bold text-gray-900">Bhava +</div>
                    <div className="text-[10px] text-amber-600">Bhavesha + Karaka</div>
                  </div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center">
                    <Zap className="w-5 h-5 text-amber-600 mx-auto mb-1" />
                    <div className="text-lg font-bold text-gray-900">3/3</div>
                    <div className="text-[10px] text-amber-600">Synthesis Check</div>
                  </div>
                </div>
              </motion.div>
              <CalloutBlock variant="important" className="mt-5">
                This lesson introduces the Trinity of Execution — the synthesis framework that combines Bhava, Bhavesha, and Karaka into a single scoring system to judge how powerfully any life matter will manifest.
              </CalloutBlock>
            </section>

            {/* ─── SECTIONS ─── */}
            {content.sections?.map((section, idx) => (
              <section key={section.id} id={`sec-${section.type === "content" ? (idx === 2 ? "pillars" : idx === 3 ? "scoring" : section.type) : section.type}`} className="mb-6 scroll-mt-32">
                <motion.div {...fadeUp}>
                ...
                {/* Interactive Trinity Execution Dashboard */}
                {idx === 2 && (
                  <motion.div {...fadeUp} className="mt-4">
                    <TrinityExecutionDashboard />
                  </motion.div>
                )}
              </section>
            ))}
                ...
                <div className="p-6 sm:p-8 bg-white rounded-2xl border-2 border-amber-200/60 shadow-sm">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xl font-bold text-gray-900">Continue to Module 4: The Timing Engines</p>
                      <p className="text-sm text-gray-500 mt-1">Next: Learn how planetary periods and transits unlock the timing of events.</p>
                    </div>
