...
import HybridBiWheelExplorer from "./HybridBiWheelExplorer";
...
interface Lesson35InteractiveProps {
  lesson: Lesson;
...
const SIDEBAR_SECTIONS: SidebarSection[] = [
  ...
  { id: "sec-definition", label: "Hybrid Approach", type: "definition", group: "Learn" },
  { id: "sec-etymology", label: "Etymology", type: "etymology", group: "Learn" },
  { id: "sec-mechanics", label: "Outer Planets", type: "mechanics", group: "Learn" },
  { id: "sec-software", label: "Bi-Wheel UI", type: "software_logic", group: "Learn" },
...
const FLASHCARDS = [
  { id: "f1", front: "What is the Hybrid Approach?", back: "Algorithmically integrating Western astrology mechanics (outer planets + progressions) into a strict Sidereal (Vedic) engine, creating a unified cross-system reading framework.", category: "Core Definition" },
  { id: "f2", front: "Which outer planets does Western astrology use?", back: "Uranus, Neptune, and Pluto. Vedic traditionally uses only the 7 visible planets (Sun through Saturn) plus the 2 lunar nodes (Rahu and Ketu).", category: "Outer Planets" },
  { id: "f3", front: "What is a Bi-Wheel overlay?", back: "A UI showing the natal chart (inner wheel) and a progression/transit chart (outer wheel) simultaneously, allowing direct visual comparison of planetary positions.", category: "Bi-Wheel" },
  { id: "f4", front: "What are Secondary Progressions?", back: "A Western technique where each day after birth represents one year of life. The progressed planetary positions are overlayed on the natal chart to time events.", category: "Progressions" },
  { id: "f5", front: "What is a Hybrid Consensus Alert?", back: "When both Vedic and Western systems agree on a prediction outcome, the confidence score is boosted. Disagreement flags the need for deeper analysis.", category: "Consensus" },
  { id: "f6", front: "What do Uranus, Neptune, and Pluto represent in a Vedic context?", back: "Uranus = sudden disruption and innovation. Neptune = dissolution, mysticism, and spiritual transcendence. Pluto = deep transformation, power, and hidden forces.", category: "Significations" },
];

const KNOWLEDGE_CHECKS = [
  { id: "kc1", question: "Which planets are considered 'outer planets' in Western astrology?", options: ["Jupiter, Saturn, Uranus", "Uranus, Neptune, Pluto", "Mars, Jupiter, Saturn", "Rahu, Ketu, Pluto"], correctIndex: 1, explanation: "Western astrology uses Uranus, Neptune, and Pluto as outer planets. Vedic traditionally uses only the 7 visible planets plus the 2 lunar nodes." },
  { id: "kc2", question: "In Secondary Progressions, what does one day after birth equal?", options: ["One month of life", "One year of life", "One decade of life", "One hour of life"], correctIndex: 1, explanation: "Secondary Progressions: each day after birth represents one year of life. This allows astrologers to progress the natal chart forward to time life events." },
  { id: "kc3", question: "What happens when Vedic and Western systems agree on a prediction?", options: ["The prediction is discarded", "A Hybrid Consensus Alert boosts confidence", "Only the Vedic result is used", "Only the Western result is used"], correctIndex: 1, explanation: "When both systems converge on the same outcome, a Hybrid Consensus Alert boosts the confidence score. When they disagree, the dashboard flags the conflict for deeper analysis." },
];
...
export default function Lesson35Interactive({ lesson, lessonProgress }: Lesson35InteractiveProps) {
  const { user } = useAuth();
  ...
  const recapItems = useMemo(() => {
    ...
    if (content.intro) items.push({ id: "recap-intro", title: "Bridging Vedic & Western", summary: "Algorithmic integration of Western outer planets and progressions into a strict Sidereal engine, with Bi-Wheel UI and Hybrid Consensus Alerts." });
    content.sections?.forEach((s) => {
      if (s.type === "definition") items.push({ id: `recap-${s.id}`, title: s.title, summary: "The Hybrid Approach combines Western mechanics (Uranus, Neptune, Pluto + progressions) with Vedic Sidereal fundamentals." });
      if (s.type === "etymology") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Bridging = connecting two systems. Hybrid = offspring of two different sources." });
      if (s.type === "mechanics") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Outer planets add generational/spiritual context. Secondary Progressions: 1 day = 1 year. Bi-Wheel overlays natal + progressed charts." });
      if (s.type === "software_logic") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Software renders Bi-Wheel UI, computes Hybrid Consensus Alerts when systems agree, and flags conflicts with weighted probabilities." });
    });
            ...
            <section id="sec-overview" className="mb-6 scroll-mt-32">
              <motion.div {...fadeUp} className="bg-white border border-amber-200/80 rounded-2xl p-6 sm:p-8 shadow-sm">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center"><Globe className="w-5 h-5 text-amber-600 mx-auto mb-1" /><div className="text-lg font-bold text-gray-900">2</div><div className="text-[10px] text-amber-600">Systems</div></div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center"><Star className="w-5 h-5 text-amber-600 mx-auto mb-1" /><div className="text-lg font-bold text-gray-900">3</div><div className="text-[10px] text-amber-600">Outer Planets</div></div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center"><Zap className="w-5 h-5 text-amber-600 mx-auto mb-1" /><div className="text-lg font-bold text-gray-900">1d=1y</div><div className="text-[10px] text-amber-600">Progressions</div></div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center"><Target className="w-5 h-5 text-amber-600 mx-auto mb-1" /><div className="text-lg font-bold text-gray-900">Bi-Wheel</div><div className="text-[10px] text-amber-600">Overlay UI</div></div>
                </div>
              </motion.div>
              <CalloutBlock variant="important" className="mt-5">This lesson teaches you to bridge two great astrological traditions. You will learn when Western outer planets add value to a Vedic reading, and how to build a software UI that renders both systems simultaneously.</CalloutBlock>
            </section>
              ...
              <motion.div {...fadeUp}>
                <div className="p-6 sm:p-8 bg-white rounded-2xl border-2 border-amber-200/60 shadow-sm">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div><p className="text-sm text-amber-600 mb-1 font-medium">🎉 Lesson Complete!</p><p className="text-xl font-bold text-gray-900">Continue to The Master Dashboard</p><p className="text-sm text-gray-500 mt-1">Next: The ultimate synthesis — a visual temporal funnel converging three prediction engines into a single high-confidence timestamp.</p></div>
