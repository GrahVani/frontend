...
import {
  ...
  Lightbulb, Play, Star, Zap, LayoutDashboard,
} from "lucide-react";
...
import MasterDashboardExplorer from "./MasterDashboardExplorer";
...
interface Lesson36InteractiveProps {
  lesson: Lesson;
...
const SIDEBAR_SECTIONS: SidebarSection[] = [
  ...
  { id: "sec-definition", label: "Master Dashboard", type: "definition", group: "Learn" },
  { id: "sec-etymology", label: "Etymology", type: "etymology", group: "Learn" },
  { id: "sec-mechanics", label: "Temporal Funnel", type: "mechanics", group: "Learn" },
  { id: "sec-software", label: "Synthesis Engine", type: "software_logic", group: "Learn" },
...
const FLASHCARDS = [
  { id: "f1", front: "What is the Master Dashboard?", back: "The ultimate algorithmic convergence UI that processes queries through multiple prediction engines to output a single high-confidence prediction timestamp.", category: "Core Definition" },
  { id: "f2", front: "What are the three engines in the temporal funnel?", back: "Vimshottari Dasha (broad time periods) → Ashtakavarga (strength scoring) → KP Sub-Lords (precise cuspal timing). Each layer narrows the prediction window.", category: "Three Engines" },
  { id: "f3", front: "What is the Temporal Funnel?", back: "A visual pipeline where broad time ranges (Dasha periods of years) are progressively narrowed through finer-grained filters (Ashtakavarga months → KP sub-lord days/hours).", category: "Temporal Funnel" },
  { id: "f4", front: "What is a High-Confidence Prediction Timestamp?", back: "A specific date/time window where all three engines converge on the same outcome. When engines agree, confidence is maximized. When they disagree, conflict is flagged.", category: "Confidence" },
  { id: "f5", front: "What happens when prediction engines disagree?", back: "The dashboard flags the conflict and provides a weighted probability based on each engine's historical accuracy for that specific query type. No false certainty.", category: "Disagreement" },
  { id: "f6", front: "What is the Synthesis Wheel?", back: "A circular UI showing all three engine outputs simultaneously, with overlapping agreement zones highlighted visually. It makes convergence and divergence instantly visible.", category: "Synthesis Wheel" },
];

const KNOWLEDGE_CHECKS = [
  { id: "kc1", question: "What are the three engines in the Master Dashboard funnel?", options: ["Gochara, Muhurtha, Prashna", "Vimshottari Dasha, Ashtakavarga, KP Sub-Lords", "Jaimini, Tajik, Varshaphala", "Rasi, Navamsa, Dasamsa"], correctIndex: 1, explanation: "The three engines are Vimshottari Dasha (broad periods), Ashtakavarga (strength scoring), and KP Sub-Lords (precise timing). They form a narrowing funnel from years to days/hours." },
  { id: "kc2", question: "What does the Temporal Funnel do?", options: ["Predicts only birth charts", "Narrows broad time ranges into specific high-confidence timestamps", "Calculates only planetary strengths", "Shows only house placements"], correctIndex: 1, explanation: "The Temporal Funnel starts with broad Dasha periods (years), filters through Ashtakavarga strength (months), and narrows to KP sub-lord precision (days/hours)." },
  { id: "kc3", question: "What happens when the three engines disagree on a prediction?", options: ["The software hides the disagreement", "A conflict is flagged with weighted probabilities", "Only the first engine's result is shown", "The user must restart the query"], correctIndex: 1, explanation: "When engines disagree, the dashboard flags the conflict and provides a weighted probability based on each engine's historical accuracy for that query type. Transparency over false certainty." },
];
...
export default function Lesson36Interactive({ lesson, lessonProgress }: Lesson36InteractiveProps) {
  const { user } = useAuth();
  ...
  const recapItems = useMemo(() => {
    ...
    if (content.intro) items.push({ id: "recap-intro", title: "The Master Dashboard", summary: "The ultimate algorithmic convergence UI. Three engines funnel into a single high-confidence prediction timestamp with full transparency on disagreement." });
    content.sections?.forEach((s) => {
      if (s.type === "definition") items.push({ id: `recap-${s.id}`, title: s.title, summary: "A unified dashboard that processes queries through Vimshottari, Ashtakavarga, and KP engines simultaneously." });
      if (s.type === "etymology") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Synthesis = combining parts into a whole. Dashboard = a visual control panel. The unified control center for all predictions." });
      if (s.type === "mechanics") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Temporal Funnel: Dasha (years) → Ashtakavarga (months) → KP Sub-Lords (days/hours). Each layer narrows the window." });
      if (s.type === "software_logic") items.push({ id: `recap-${s.id}`, title: s.title, summary: "Software runs parallel queries, renders the Synthesis Wheel, computes consensus scores, and flags conflicts with weighted probabilities." });
    });
            ...
            <section id="sec-overview" className="mb-6 scroll-mt-32">
              <motion.div {...fadeUp} className="bg-white border border-amber-200/80 rounded-2xl p-6 sm:p-8 shadow-sm">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center"><LayoutDashboard className="w-5 h-5 text-amber-600 mx-auto mb-1" /><div className="text-lg font-bold text-gray-900">3</div><div className="text-[10px] text-amber-600">Engines</div></div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center"><Star className="w-5 h-5 text-amber-600 mx-auto mb-1" /><div className="text-lg font-bold text-gray-900">Funnel</div><div className="text-[10px] text-amber-600">Temporal</div></div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center"><Zap className="w-5 h-5 text-amber-600 mx-auto mb-1" /><div className="text-lg font-bold text-gray-900">High</div><div className="text-[10px] text-amber-600">Confidence</div></div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-3 text-center"><Target className="w-5 h-5 text-amber-600 mx-auto mb-1" /><div className="text-lg font-bold text-gray-900">Wheel</div><div className="text-[10px] text-amber-600">Synthesis</div></div>
                </div>
              </motion.div>
              <CalloutBlock variant="important" className="mt-5">This is the capstone lesson of the entire curriculum. You will learn how to build a master-level prediction dashboard that converges multiple engines into a single, transparent, high-confidence output. This is what separates apprentice software from master-level astro-engineering.</CalloutBlock>
            </section>
              ...
              <motion.div {...fadeUp}>
                <div className="p-6 sm:p-8 bg-white rounded-2xl border-2 border-amber-200/60 shadow-sm">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div><p className="text-sm text-amber-600 mb-1 font-medium">🎓 Curriculum Complete!</p><p className="text-xl font-bold text-gray-900">You Are Now a Master Astro-Engineer</p><p className="text-sm text-gray-500 mt-1">You have mastered all 10 modules — from celestial geometry to multi-system synthesis. Your journey continues with real chart readings and building production astrology software.</p></div>
                    <Link href="/learn" className="flex items-center gap-2 px-6 py-3 bg-amber-600 hover:bg-amber-700 text-white font-semibold rounded-xl transition-colors shadow-md shadow-amber-600/20 shrink-0">Back to Learning Path <ChevronRight className="w-4 h-4" /></Link>
                  </div>
                </div>
              </motion.div>
            </section>
