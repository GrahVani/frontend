      ...
      <div className="rounded-3xl bg-gradient-to-br from-slate-50 via-amber-50/30 to-orange-50/20 border border-amber-200/50 shadow-2xl shadow-amber-900/5 p-6 sm:p-8">
        ...
        <div className="flex items-center justify-center gap-2 mb-6">
          {TIERS.map((t, i) => (
            ...
            >
              ...
              <div className={`text-[9px] font-medium ${tier === t.key ? "text-amber-100" : "text-gray-400"}`}>
                {t.sanskrit}
              </div>
            </button>
          ))}
        ...
        <div className="mb-5 p-4 rounded-2xl bg-white border border-amber-100 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs font-bold text-amber-700 uppercase tracking-wider">Anchor Point: Moon's Nakshatra at Birth</span>
          </div>
        ...
        <div className="mb-5">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">120-Year Timeline</span>
            <span className="text-[10px] text-gray-400">Birth → Age 120</span>
          </div>
          <div className="relative h-14 bg-gray-100 rounded-2xl overflow-hidden flex shadow-inner">
            {sequence.map((p, i) => {
              ...
              return (
                ...
                >
                  <span className="font-bold text-white text-xs">{p.planet[0]}</span>
                  {widthPct > 6 && (
                    <span className="text-[9px] text-white/80 font-semibold">{p.years}y</span>
                  )}
                </button>
              );
            })}
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-gray-400">{tier === "antardasha" ? "Sub-periods within this Mahadasha" : "Sub-sub-periods for precise timing"}</div>
                  </div>
                </div>
                <button
                  onClick={() => setShowMath(!showMath)}
                  className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-amber-50 border border-amber-200 text-amber-700 text-[11px] font-bold hover:bg-amber-100 transition-colors"
                >
                  ...
                  getAntardashas(selectedPlanet).map((ad, i) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[10px] text-gray-500">{ad.planet.sanskrit}</div>
                      </div>
                      <div className="text-right shrink-0">
                        <div className="text-sm font-extrabold" style={{ color: ad.planet.color }}>{ad.years}y</div>
                        <div className="text-[9px] text-gray-400">{Math.round(ad.years * 12)} months</div>
                      </div>
                    </div>
                  ))
                ) : (
            ...
            <motion.div {...fadeUp} className="p-5 rounded-2xl border shadow-sm" style={{ background: selectedPlanet.lightColor, borderColor: `${selectedPlanet.color}30` }}>
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500 mb-2">{selectedPlanet.sanskrit} · {selectedPlanet.years} Years</p>
                  <p className="text-sm text-gray-700 leading-relaxed">{selectedPlanet.description}</p>
                  {tier === "mahadasha" && (
                    <div className="mt-3 flex items-center gap-2">
                      <button
                        onClick={() => setTier("antardasha")}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white border border-amber-200 text-amber-700 text-xs font-bold hover:bg-amber-50 transition-colors shadow-sm"
                      >
                        Drill into Antardashas <ChevronRight className="w-3 h-3" />
                      </button>
                    </div>
                  )}
                </div>
