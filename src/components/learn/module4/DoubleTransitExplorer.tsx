...
export default function DoubleTransitExplorer() {
  ...
  return (
    <div className="w-full max-w-[780px] mx-auto select-none">
      <div className="rounded-3xl bg-gradient-to-br from-slate-50 via-amber-50/30 to-orange-50/20 border border-amber-200/50 shadow-2xl shadow-amber-900/5 p-6 sm:p-8">
        {/* Header */}
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 mb-5">
          <div className="flex items-center gap-2 bg-white rounded-xl px-3 py-2 border border-orange-200 shadow-sm">
            <span className="text-[10px] font-bold text-orange-600 uppercase tracking-wider">Jupiter</span>
            <select
              value={jupiterPos}
          ...
          <div className="flex items-center gap-2 bg-white rounded-xl px-3 py-2 border border-slate-200 shadow-sm">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Saturn</span>
            <select
              value={saturnPos}
        ...
        <div className="relative w-full max-w-[400px] mx-auto aspect-square">
          <svg viewBox="0 0 400 400" className="w-full h-full">
            ...
            {(() => {
              ...
              return (
                <g>
                  ...
                  <text x={pos.x} y={pos.y + 26} textAnchor="middle" fontSize={9} fontWeight={700} fill="#f97316">Jupiter</text>
                </g>
              );
            })()}
            ...
            {(() => {
              ...
              return (
                <g>
                  ...
                  <text x={pos.x} y={pos.y} textAnchor="middle" dominantBaseline="central" fontSize={10} fontWeight={900} fill="#fff">♄</text>
                  <text x={pos.x} y={pos.y + 26} textAnchor="middle" fontSize={9} fontWeight={700} fill="#64748b">Saturn</text>
                </g>
              );
            })()}
            ...
            <text x={cx} y={cy - 3} textAnchor="middle" dominantBaseline="central" fontSize={9} fontWeight={800} fill="#92400e">Double</text>
            <text x={cx} y={cy + 10} textAnchor="middle" dominantBaseline="central" fontSize={8} fill="#78350f">Transit</text>
          </svg>
        </div>
        ...
        <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
          {doubleHitHouses.length > 0 ? (
            ...
            <span className="text-xs text-gray-400 italic">No double transit active with current positions.</span>
          )}
        </div>
        ...
        <div className="mt-4 p-4 rounded-2xl bg-white border border-gray-100 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-bold text-amber-700 uppercase tracking-wider">The Rule</span>
          </div>
        ...
        <div className="mt-3 p-4 rounded-2xl bg-amber-50/60 border border-amber-200/40">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-bold text-amber-700 uppercase tracking-wider">Advanced: House Lord Vector</span>
          </div>
          <p className="text-sm text-gray-600 leading-relaxed">
                ...
                return (
                  <div className={`p-4 rounded-2xl border shadow-sm ${
                    ...
                    <div className="flex flex-wrap gap-2">
                      {hasJ && (
                        <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full bg-orange-50 text-orange-600 border border-orange-100">
                          <CheckCircle2 className="w-3 h-3" /> Jupiter aspects this house
                        </span>
                      )}
                      {hasS && (
                        <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full bg-slate-100 text-slate-600 border border-slate-200">
                          <CheckCircle2 className="w-3 h-3" /> Saturn aspects this house
                        </span>
                      )}
                      {isDouble && (
                        <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full bg-violet-100 text-violet-700 border border-violet-200">
                          <Crosshair className="w-3 h-3" /> Double Transit Active — Event Window Open
                        </span>
                      )}
                      {!hasJ && !hasS && (
                        <span className="text-xs text-gray-400">No planet aspects this house currently.</span>
                      )}
                    </div>
                  </div>
                );
        ...
        <div className="mt-4 flex items-start gap-3 p-4 rounded-2xl bg-amber-50/60 border border-amber-200/40">
