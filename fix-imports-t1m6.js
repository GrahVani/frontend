...
const moduleDir = 'src/components/learning-runtime/interactive/tier-1/module-6';
...
for (const file of files) {
  ...
  while ((match = regex.exec(content)) !== null) {
    ...
    // Check if it exists in module-6
    const inModule6 = fs.existsSync(path.join(moduleDir, componentName));
    ...
    // Wait! A component might have ALREADY been moved to module-4 or module-5.
    // E.g., `useLessonSlug` from `rashi-attribute-wheel` (moved to module-4).
    // The previous regex replacement I did in module-4 was:
    // `content.replace(/\.\.\/rashi-attribute-wheel/g, '@/components/learning-runtime/interactive/tier-1/module-4/rashi-attribute-wheel');`
    // I already did a system-wide replacement for `rashi-attribute-wheel` so that one is fine.
    
    if (!inModule6 && inInteractive) {
